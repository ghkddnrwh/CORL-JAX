# JAX/Flax Leave-One-Out Decoupled Delayed IQL implementation with CDAF_JAX-style experiment plumbing.
#
# LDDIQL idea:
...
#     computed from the leave-one-out average of all other delayed pairs:
#     mean_{k != i}(Q_k_delayed - V_k_delayed).
#   - If N=1, there is no other member, so the code falls back to the member's
#     own delayed advantage. If N>=2, each member's filter excludes itself.
...
ALGORITHM_NAME = "LDDIQL"
ALGORITHM_FULL_NAME = "Leave-One-Out Decoupled Delayed Implicit Q-Learning"
...
class TrainConfig:
    ...
    hyperparams_path: Optional[str] = "hyperparams/lddiql_jax.yml"
...
class LDDIQLJAX:
    """Leave-One-Out Decoupled Delayed IQL in JAX/Flax.

    For each ensemble member i:

        Q_i target: r + gamma * V_i(s')
        V_i target: Q_i_target(s, a)
        V_i weight: |tau - 1[mean_{k != i}(Q_k_delayed(s, a) - V_k_delayed(s)) < 0]|

    If N=1, there is no other member, so the code falls back to the member's
    own delayed advantage. If N>=2, the expectile weight for member i is
    computed from the average delayed advantage of every other member, excluding i.
    ...
    def _build_train_step(self):
        ...
        def cycle_shift_for_update(total_it_: jnp.ndarray) -> jnp.ndarray:
            # Kept only for backward-compatible state/checkpoint structure.
            # The expectile filtering weight below no longer uses a single
            # cyclic partner; it uses a leave-one-out average over all other
            # delayed ensemble members.
            if ensemble_size == 1:
                return jnp.zeros((1,), dtype=jnp.int32)
        ...
        def leave_one_out_mean(x: jnp.ndarray) -> jnp.ndarray:
            # x has shape [N, B]. For each ensemble member i, return the
            # average over all k != i. For N=1, fall back to x itself.
            if ensemble_size == 1:
                return x
            denom = jnp.asarray(ensemble_size - 1, dtype=x.dtype)
            return (jnp.sum(x, axis=0, keepdims=True) - x) / denom

        @jax.jit
        def train_step(state: LDDIQLState, batch: TensorBatch):
            ...
            # Decoupled delayed filtering weights:
            # target member i uses the leave-one-out average of all delayed
            # members except itself, instead of a single delayed partner j(i).
            ...
            delayed_adv_all = delayed_q_all - delayed_v_all  # [N, B]
            delayed_adv_for_filter = leave_one_out_mean(delayed_adv_all)  # [N, B]
            ...
            log_dict = {
                ...
                "leave_one_out_filtering": jnp.asarray(1.0, dtype=jnp.float32),
            }
    ...
    def _build_actor_refit_step(self):
        ...
        @jax.jit
        def actor_refit_step(actor_state: ActorState, iql_state: LDDIQLState, batch: TensorBatch):
            observations = batch["observations"]
            actions = batch["actions"]
    ...
    def fit_actor(
        ...
        for fit_step in range(1, steps + 1):
            ...
            if should_eval:
                ...
                print(
                    f"[{prefix}:lddiql_awbc] step {fit_step}/{steps}: "
    ...
    def state_dict(self) -> Dict[str, Any]:
        return {
            "lddiql_state": serialization.to_state_dict(self.state),
            # Backward-compatible alias for checkpoints/scripts that still expect
            # the previous internal key.
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]):
        iql_state_key = (
            "lddiql_state"
            if "lddiql_state" in state_dict
            else "decoupled_delayed_iql_state"
        )
        self.state = serialization.from_state_dict(
            self.state,
            state_dict[iql_state_key],
        )
...
def resolve_checkpoint_path(
    ...
    """Return (run_dir, checkpoint_path) for a saved LDDIQL-JAX checkpoint.
...
def _train_impl(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="ldd_iql_jax_training_progress",
