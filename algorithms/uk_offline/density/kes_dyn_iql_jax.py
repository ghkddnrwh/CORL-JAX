# kes_dyn_iql_jax.py
#
# KES-Dyn-IQL: Dynamics-Certified Kernel Effective-Support IQL (JAX/Flax).
#
...
#     w_ij = K_ij * g_ij,
# where K_ij is the distance kernel and g_ij is a per-edge dynamics-consistency
# gate from the local linear dynamics residual. No state-level junction gate and
# no neighbor Q/action pooling are used.
#
import copy
import json
import os
...
ALGORITHM_NAME = "KES-Dyn-IQL"
ALGORITHM_FULL_NAME = "Dynamics-Certified Kernel Effective-Support IQL"
...
class TrainConfig:
    ...
    # ----- KES-Dyn-IQL: dynamics-certified kernel effective-support value learning --------------
    # Neighbor Q/action pooling is not used. Dynamics-certified kernel weights over the kNN graph
...
def build_neighbor_weights(
    ...
    """Build dynamics-certified kernel support weights for KES-Dyn-IQL.
    ...
    Returns:
        ...
        neighbor_weights (N, k) float32
            = K_ij * g_ij, where
              K_ij = exp(-(d_ij / h_i)^2)
              g_ij = exp(-(r_ij / dcs_dynamics_scale)^2).
            No junction/state gate G_i is applied in KES-Dyn-IQL.
        support_proxy    (N,) float32
            logging-only mean dynamics gate per state, stored in the existing
            auxiliary slot.

    Important: N_k(i) has already been selected by coverage_profile in the
    chosen metric space. This function only converts distances and local
    dynamics residuals into the support weight used for the effective-support
    estimate.
    """
    ...
    if config.dcs_use_dynamics_gate and "dynamics_residual" in profile:
        ...
        neighbor_weights = (kernel * dynamics_g).astype(np.float32)
        support_proxy = np.mean(dynamics_g, axis=1).astype(np.float32)
    else:
        if config.dcs_use_dynamics_gate:
            print(
                "dcs_use_dynamics_gate=True but the cached profile has no dynamics_residual; "
                "KES-Dyn-IQL is falling back to kernel-only weights. Recompute the profile "
                "with --dcs_force_recompute True if this cache is stale."
            )
        dynamics_g = np.ones_like(kernel, dtype=np.float32)
        neighbor_weights = kernel.astype(np.float32)
        support_proxy = np.ones((kernel.shape[0],), dtype=np.float32)
    ...
    print(
        "KES-Dyn support profile | "
        f"weights=K_ij*g_ij | mean edge weight={mean_weight:.4f} | "
        f"mean per-state mass={mean_mass:.4f} | "
        f"active-row frac={active_row_frac:.4f} | active-edge frac={active_edge_frac:.4f}"
    )
    print(
        f"Dynamics gate | mean={float(np.mean(dynamics_g)):.4f} | "
        f"std={float(np.std(dynamics_g)):.4f} | "
        f"active-edge frac={float(np.mean(dynamics_g > 1e-6)):.4f} | "
        f"dcs_dynamics_scale={config.dcs_dynamics_scale}"
    )
    print(
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="kes_dyn_iql_jax_training_progress",
