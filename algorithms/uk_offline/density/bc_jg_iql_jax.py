# bc_jg_iql_jax.py
#
# BC-JG-IQL: Branch-Coreset Junction-Gated Implicit Q-Learning (JAX/Flax).
#
# Computationally efficient variant of JG-IQL. The original JG-IQL builds an
# exact full-dataset kNN graph only to obtain a scalar state gate G_i. BC-JG-IQL
# removes that graph entirely and estimates the same branching evidence through:
#
#   1) a compact set of state-space prototypes c_m,
#   2) a small branch-aware coreset of real transitions per prototype cell,
#   3) soft relational comparisons against only the nearby prototype coresets.
#
# For each state i, the method estimates
#   A_i : local action diversity,
#   P_i : local continuation/displacement diversity,
# and by default uses
#   J_i = sqrt(A_i * P_i).
#
# The gate is then calibrated automatically with a two-component Beta mixture:
#   G_i = P(high-mean mixture component | J_i),
#   c_i = G_i ** p,
#   tau_i = tau_min + (tau_max - tau_min) * c_i.
#
# No N x k neighbor graph, neighbor distances, neighbor Q pooling, or neighbor
# action copying are used. Q-learning and actor extraction remain standard IQL.
#
import copy
import hashlib
import json
...
ALGORITHM_NAME = "BC-JG-IQL"
ALGORITHM_FULL_NAME = "Branch-Coreset Junction-Gated Implicit Q-Learning"
...
class TrainConfig:
    ...
    # ----- BC-JG-IQL: branch-coreset-gated value learning ----------------------
    # Neighbor Q/action pooling is not used. A graph-free state gate G_i controls
    ...
    # ----- Branching gate ------------------------------------------------
    # Master switch. False -> no branch-coreset preprocessing and
    # tau(s)=scv_tau_min, equivalent to standard IQL when
    # scv_tau_min == scv_tau_max.
    use_dcs: bool = True

    # Legacy compatibility only. BC-JG-IQL does not build or use an N x k graph.
    dcs_k: int = 8
    ...
    # ----- Branch-Coreset approximation -----------------------------------
    # Number of state-space prototypes. Every dataset state is assigned to its
    # nearest prototype; exact sample-to-sample kNN search is never performed.
    bc_num_prototypes: int = 256

    # Prototype fitting uses at most this many states and streaming mini-batch
    # k-means. The final gate is still inferred for every dataset state.
    bc_prototype_fit_samples: int = 100_000
    bc_kmeans_iters: int = 10
    bc_kmeans_batch_size: int = 4096

    # Number of real branch-diverse transitions retained in each prototype cell.
    bc_coreset_size: int = 32

    # Farthest-first branch selection is performed on at most this many candidate
    # transitions per prototype cell. Half are chosen near the prototype center
    # and half randomly, preserving locality while retaining rare branches.
    bc_coreset_candidates: int = 1024

    # Each anchor consults only the coresets attached to its r nearest prototypes.
    # Candidate count per state is therefore at most r * bc_coreset_size.
    bc_nearest_prototypes: int = 2

    # Soft relational weight:
    #   alpha_ij proportional to exp(-d_ij^2 / (temperature * local_scale_i)).
    bc_relation_temperature: float = 1.0

    # Branch descriptor used only for coreset construction:
    #   b_i = [normalized_action_i, disp_weight * unit_displacement_i].
    bc_descriptor_disp_weight: float = 1.0

    # Chunk size for all-state prototype assignment and branch-profile inference.
    bc_profile_chunk_size: int = 4096

    # Seed used only for prototype / coreset preprocessing, independent of RL
    # minibatch sampling. Fixed by default so multiple RL seeds can share a cache.
    bc_preprocess_seed: int = 0

    # Cache controls. The cache stores only final per-state statistics and gate,
    # not an N x k graph.
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.bc_coreset_size >= 1
    assert config.bc_coreset_candidates >= 1
    assert config.bc_nearest_prototypes >= 1
    assert config.bc_relation_temperature > 0.0
    assert config.bc_descriptor_disp_weight >= 0.0
...
def build_branch_coreset_signature(
    config: TrainConfig,
    ...
    temporal_sig = (
        tmet.signature_hash(
            build_temporal_signature(config)
        )
        if config.dcs_metric_source == "temporal"
        else None
    )
    return {
        ...
        "coreset_size": int(config.bc_coreset_size),
        "coreset_candidates": int(config.bc_coreset_candidates),
        "nearest_prototypes": int(config.bc_nearest_prototypes),
        "relation_temperature": float(config.bc_relation_temperature),
        "descriptor_disp_weight": float(config.bc_descriptor_disp_weight),
...
def get_branch_coreset_cache_path(
    config: TrainConfig,
    metric_space: np.ndarray,
    actions: np.ndarray,
) -> Optional[Path]:
    if config.dcs_profile_path is None:
        return None
    signature = build_branch_coreset_signature(config, metric_space, actions)
    payload = json.dumps(signature, sort_keys=True, separators=(",", ":")).encode("utf-8")
    sig_hash = hashlib.sha1(payload).hexdigest()[:12]
    root = Path(config.dcs_profile_path)
    env_name = _safe_path_name(config.env)
    return root / env_name / f"branch_coreset_profile_{sig_hash}.npz"
...
def _fit_streaming_prototypes(
    ...
    Complexity is O(S * M * I), where S is the capped fitting subset and
    M << N. This replaces exact O(N^2)-style sample-to-sample neighbor search.
    """
...
def _branch_aware_coreset_indices(
    metric_space: np.ndarray,
    centers: np.ndarray,
    assignments: np.ndarray,
    nearest_d2: np.ndarray,
    actions: np.ndarray,
    unit_disp: np.ndarray,
    coreset_size: int,
    candidate_budget: int,
    disp_weight: float,
    seed: int,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Keep a small set of real, branch-diverse transitions per prototype cell."""
    n_prototypes = centers.shape[0]
    b = int(coreset_size)
    candidate_budget = max(int(candidate_budget), b)
    rng = np.random.default_rng(seed)

    action_mean = np.mean(actions, axis=0, keepdims=True)
    action_std = np.std(actions, axis=0, keepdims=True) + 1e-6

    order = np.argsort(assignments, kind="stable")
    counts = np.bincount(assignments, minlength=n_prototypes).astype(np.int64)
    offsets = np.concatenate([[0], np.cumsum(counts)])

    reps = np.full((n_prototypes, b), -1, dtype=np.int32)
    valid = np.zeros((n_prototypes, b), dtype=bool)
    cell_radius = np.zeros((n_prototypes,), dtype=np.float32)

    for m in range(n_prototypes):
        members = order[offsets[m] : offsets[m + 1]]
        if members.size == 0:
            continue

        cell_radius[m] = float(np.sqrt(np.mean(nearest_d2[members]) + 1e-12))

        if members.size > candidate_budget:
            local_d2 = nearest_d2[members]
            n_close = max(1, candidate_budget // 2)
            close_local = np.argpartition(local_d2, n_close - 1)[:n_close]
            close_members = members[close_local]

            remaining_mask = np.ones((members.size,), dtype=bool)
            remaining_mask[close_local] = False
            remaining = members[remaining_mask]
            n_random = candidate_budget - close_members.size
            random_members = rng.choice(
                remaining,
                size=min(n_random, remaining.size),
                replace=False,
            )
            candidates = np.concatenate([close_members, random_members])
        else:
            candidates = members

        a_desc = (actions[candidates] - action_mean) / action_std
        descriptor = np.concatenate(
            [a_desc, float(disp_weight) * unit_disp[candidates]], axis=1
        ).astype(np.float32)

        take = min(b, candidates.size)
        if take == candidates.size:
            selected_local = np.arange(take)
        else:
            # Start from the transition furthest from the local descriptor mean,
            # then use farthest-first traversal to preserve rare branch modes.
            desc_mean = np.mean(descriptor, axis=0, keepdims=True)
            first = int(np.argmax(np.sum((descriptor - desc_mean) ** 2, axis=1)))
            selected_local = np.empty((take,), dtype=np.int32)
            selected_local[0] = first
            min_d2 = np.sum((descriptor - descriptor[first]) ** 2, axis=1)
            min_d2[first] = -np.inf
            for t in range(1, take):
                nxt = int(np.argmax(min_d2))
                selected_local[t] = nxt
                d2_new = np.sum((descriptor - descriptor[nxt]) ** 2, axis=1)
                min_d2 = np.minimum(min_d2, d2_new)
                min_d2[selected_local[: t + 1]] = -np.inf

        chosen = candidates[selected_local]
        reps[m, :take] = chosen.astype(np.int32)
        valid[m, :take] = True

    return reps, valid, counts.astype(np.float32), cell_radius
...
def _softmax_masked(logits: np.ndarray, valid: np.ndarray) -> np.ndarray:
    ...
    exp_logits = np.where(valid, np.exp(logits - row_max), 0.0)
    denom = np.sum(exp_logits, axis=1, keepdims=True)
    return np.where(denom > 0.0, exp_logits / np.maximum(denom, 1e-12), 0.0)


def compute_branch_coreset_profile(
    config: TrainConfig,
    dataset: Dict[str, np.ndarray],
    metric_space: np.ndarray,
) -> Dict[str, Any]:
    """Estimate local branching evidence without any full-dataset kNN graph."""
    ...
    print(
        f"Building branch-coreset profile: N={n}, prototypes={n_prototypes}, "
        f"coreset/prototype={config.bc_coreset_size}, nearby_prototypes={n_near}, "
        f"max_candidates/state={n_near * config.bc_coreset_size}"
    )
    ...
    coreset_idx, coreset_valid, cell_counts, cell_radius = _branch_aware_coreset_indices(
        ...
        actions=actions,
        unit_disp=unit_disp,
        coreset_size=config.bc_coreset_size,
        candidate_budget=config.bc_coreset_candidates,
        disp_weight=config.bc_descriptor_disp_weight,
        seed=config.bc_preprocess_seed,
    )

    action_spread = np.zeros((n,), dtype=np.float32)
    disp_diversity = np.zeros((n,), dtype=np.float32)
    density_proxy = np.zeros((n,), dtype=np.float32)
    effective_ref_count = np.zeros((n,), dtype=np.float32)

    # Compact-cell density proxy. It is used only when a legacy density-based
    # gate_source is explicitly requested; product_diversity remains the default.
    positive_radius = cell_radius[cell_radius > 1e-8]
    radius_floor = float(np.median(positive_radius)) if positive_radius.size else 1.0
    safe_radius = np.maximum(cell_radius, max(radius_floor * 1e-3, 1e-6))
    prototype_density = cell_counts / safe_radius

    for start in range(0, n, int(config.bc_profile_chunk_size)):
        end = min(start + int(config.bc_profile_chunk_size), n)
        anchors = metric_space[start:end]
        batch_indices = np.arange(start, end, dtype=np.int64)
        center_d2 = _squared_distance_matrix(anchors, centers)

        if n_near == 1:
            top_proto = np.argmin(center_d2, axis=1)[:, None]
        else:
            top_proto = np.argpartition(center_d2, n_near - 1, axis=1)[:, :n_near]
            top_d2 = np.take_along_axis(center_d2, top_proto, axis=1)
            rank = np.argsort(top_d2, axis=1)
            top_proto = np.take_along_axis(top_proto, rank, axis=1)

        cand_idx = coreset_idx[top_proto].reshape(end - start, -1)
        cand_valid = coreset_valid[top_proto].reshape(end - start, -1)
        safe_idx = np.maximum(cand_idx, 0)

        rep_z = metric_space[safe_idx]
        d2 = np.sum((anchors[:, None, :] - rep_z) ** 2, axis=-1)
        valid = cand_valid & (cand_idx != batch_indices[:, None])

        # Per-anchor adaptive scale avoids introducing a metric-specific distance
        # threshold. Mean positive candidate distance is robust enough here because
        # the candidate set is already local and tiny.
        valid_f = valid.astype(np.float32)
        scale = np.sum(d2 * valid_f, axis=1) / np.maximum(np.sum(valid_f, axis=1), 1.0)
        scale = np.maximum(scale, 1e-8)
        logits = -d2 / (float(config.bc_relation_temperature) * scale[:, None])
        alpha = _softmax_masked(logits, valid)

        rep_actions = actions[safe_idx]
        a_mean = np.sum(alpha[:, :, None] * rep_actions, axis=1)
        a_second = np.sum(alpha * np.sum(rep_actions ** 2, axis=-1), axis=1)
        a_var = np.maximum(a_second - np.sum(a_mean ** 2, axis=-1), 0.0)
        action_spread[start:end] = np.sqrt(a_var).astype(np.float32)

        rep_u = unit_disp[safe_idx]
        u_mean = np.sum(alpha[:, :, None] * rep_u, axis=1)
        valid_count = np.sum(valid, axis=1)
        disp_chunk = np.clip(
            1.0 - np.linalg.norm(u_mean, axis=-1), 0.0, 1.0
        )
        disp_diversity[start:end] = np.where(
            valid_count > 0, disp_chunk, 0.0
        ).astype(np.float32)

        # Effective number of softly weighted references: 1 / sum_j alpha_ij^2.
        effective_ref_count[start:end] = np.where(
            np.sum(alpha ** 2, axis=1) > 1e-12,
            1.0 / np.maximum(np.sum(alpha ** 2, axis=1), 1e-12),
            0.0,
        ).astype(np.float32)

        nearest_proto = top_proto[:, 0]
        local_radius2 = np.maximum(safe_radius[nearest_proto] ** 2, 1e-8)
        density_proxy[start:end] = (
            prototype_density[nearest_proto]
            * np.exp(-center_d2[np.arange(end - start), nearest_proto] / local_radius2)
        ).astype(np.float32)

        if start == 0 or end == n or (start // int(config.bc_profile_chunk_size)) % 25 == 0:
            print(f"  branch profile: {end}/{n} states")
    ...
    return {
        "density_conf": density_conf.astype(np.float32),
        "action_div_conf": action_div_conf.astype(np.float32),
        "disp_div_conf": disp_div_conf.astype(np.float32),
        "gate_signal": np.asarray(gate_signal, dtype=np.float32),
        "gate": np.asarray(gate, dtype=np.float32),
        "action_spread_raw": action_spread,
        "disp_diversity_raw": disp_diversity,
        "density_proxy_raw": density_proxy,
        "effective_ref_count": effective_ref_count,
        "prototype_count": np.asarray([n_prototypes], dtype=np.int32),
        "nonempty_prototype_count": np.asarray([np.sum(cell_counts > 0)], dtype=np.int32),
        "mean_cell_size": np.asarray([np.mean(cell_counts)], dtype=np.float32),
        "max_cell_size": np.asarray([np.max(cell_counts)], dtype=np.float32),
        "mean_valid_coreset_size": np.asarray([np.mean(np.sum(coreset_valid, axis=1))], dtype=np.float32),
        "gate_signal_name": np.asarray([gate_signal_name]),
        "gate_diagnostics_json": np.asarray([json.dumps(gate_diagnostics, sort_keys=True)]),
    }


def load_or_compute_branch_coreset_profile(
    config: TrainConfig,
    dataset: Dict[str, np.ndarray],
    device: Any = None,
    temporal_representation: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """Load or build the graph-free branch-coreset profile."""
    if config.dcs_metric_source == "oracle":
        metric_space = build_oracle_metric_space(config, dataset)
        print("BC-JG metric space: oracle (ceiling test only).")
    elif config.dcs_metric_source == "temporal":
        ...
        print("BC-JG metric space: learned temporal representation.")
    else:
        metric_space = np.asarray(dataset["observations"], dtype=np.float32)
        print("BC-JG metric space: normalized observations.")

    cache_path = get_branch_coreset_cache_path(config, metric_space, dataset["actions"])
    signature = build_branch_coreset_signature(config, metric_space, dataset["actions"])
        ...
        try:
            with np.load(cache_path, allow_pickle=False) as cached:
                if str(cached["signature_json"].item()) == signature_json:
                    profile = {key: cached[key] for key in cached.files if key != "signature_json"}
                    print(f"Loaded branch-coreset profile from: {cache_path}")
                    return profile
        except Exception as exc:
            print(f"Failed to load branch-coreset cache {cache_path}: {exc}; recomputing.")

    profile = compute_branch_coreset_profile(config, dataset, metric_space)
    if cache_path is not None:
        cache_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        print(f"Saved branch-coreset profile to: {cache_path}")
    return profile
...
def build_branch_coreset_replay_fields(
    config: TrainConfig,
    profile: Dict[str, Any],
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Create minimal ReplayBuffer compatibility fields plus the actual gate G_i.

    BC-JG-IQL does not store or use an N x k neighbor graph. A single dummy self
    index is retained only because the shared ReplayBuffer class expects a neighbor
    axis. The trainer reads only batch["junction"] when constructing tau(s).
    """
    gate = np.asarray(profile["gate"], dtype=np.float32).reshape(-1)
    n = gate.shape[0]
    dummy_indices = np.arange(n, dtype=np.int32)[:, None]
    # Preserve existing diagnostic logging: one dummy weight equals G_i.
    ...
    effective_ref_count = np.asarray(profile["effective_ref_count"], dtype=np.float32)
    ...
    print(
        ...
        f"mean_effective_refs={float(np.mean(effective_ref_count)):.2f}"
    )
    print(
        "Branch coreset diagnostics | "
        ...
        f"mean_cell_size={float(np.asarray(profile['mean_cell_size']).reshape(-1)[0]):.2f} | "
        f"max_cell_size={float(np.asarray(profile['max_cell_size']).reshape(-1)[0]):.0f} | "
        f"mean_valid_coreset_size={float(np.asarray(profile['mean_valid_coreset_size']).reshape(-1)[0]):.2f}"
    )
    print(
        "Branch statistics | "
        f"action_conf_mean={float(np.mean(action_div_conf)):.4f} | "
        f"disp_conf_mean={float(np.mean(disp_div_conf)):.4f} | "
        f"density_proxy_conf_mean={float(np.mean(density_conf)):.4f} | "
    ...
    if gate_diagnostics.get("status") == "ok":
        print(
            ...
            f"P(high)>=0.5 frac={gate_diagnostics.get('posterior_gt_05_frac', float('nan')):.4f}"
        )
    else:
        print(
            "Gate calibration | "
...
class BranchCoresetJunctionIQLJAX:
    """Branch-Coreset Junction-Gated IQL in JAX/Flax.
    ...
    computed from graph-free local branching evidence.
    """
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="bc_jg_iql_jax_training_progress",
    ...
    # ----- BC-JG profile: prototypes + branch-aware coresets, no kNN graph ---
    neighbor_indices = None
    neighbor_weights = None
    junction = None
    buffer_neighbor_k = 1
    if config.use_dcs:
        profile = load_or_compute_branch_coreset_profile(
            config,
            dataset,
            device=jax_device,
            temporal_representation=temporal_representation,
        )
        neighbor_indices, neighbor_weights, junction = build_branch_coreset_replay_fields(
            config, profile
        )
    else:
        print(
            "use_dcs=False: branch-coreset gate disabled; tau(s)=scv_tau_min. "
    ...
    print(
        f"support_tau=[{config.scv_tau_min}, {config.scv_tau_max}] | use_branch_coreset={config.use_dcs} | "
        f"prototypes={config.bc_num_prototypes} | coreset={config.bc_coreset_size} | "
        f"near_prototypes={config.bc_nearest_prototypes} | "
    ...
    if config.use_dcs:
        ...
        if config.dcs_metric_source == "oracle":
            keys = resolve_oracle_keys(config)
            print(
                f"*** ORACLE CEILING TEST: prototypes/coresets built in ORACLE space "
                f"(keys={list(keys) if keys else 'auto'}); training still uses "
                f"standard observations. ***"
            )
        elif config.dcs_metric_source == "temporal":
            print(
                f"*** TEMPORAL METRIC: prototypes/coresets built in a LEARNED temporal "
            ...
            print("metric source: observation (prototype + branch-coreset local relations)")
    print("---------------------------------------")

    trainer = BranchCoresetJunctionIQLJAX(
