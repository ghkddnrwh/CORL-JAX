...
ALGORITHM_NAME = "GPS-IQL"
ALGORITHM_FULL_NAME = "Graph-Potential Shaped Implicit Q-Learning"
...
class TrainConfig:
    ...
    # ----- GPS-IQL: graph-potential Bellman reward shaping ----------------
    # Uses dataset stage fields such as puzzle button_states to build a directed
    # transition graph and adds lambda*(gamma*Phi(s') - Phi(s)) to rewards.
    gps_use_graph_shaping: bool = True
    gps_graph_keys: Optional[str] = None
    gps_shaping_coef: float = 1.0
    gps_normalize_potential: bool = True
    gps_boundary_tol: float = 1e-4
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.gps_shaping_coef >= 0.0
    assert config.gps_boundary_tol >= 0.0
...
# ---------------------------------------------------------------------------
# Graph potential shaping helpers (GPS-IQL)
# ---------------------------------------------------------------------------

def _resolve_graph_keys(config: TrainConfig) -> Tuple[str, ...]:
    if config.gps_graph_keys:
        return tuple(x.strip() for x in config.gps_graph_keys.split(",") if x.strip())
    keys = resolve_oracle_keys(config)
    if keys:
        return keys
    raise SystemExit(
        f"gps_use_graph_shaping=True but no graph keys could be resolved for env "
        f"'{config.env}'. Pass --gps_graph_keys, e.g. 'button_states'."
    )


def _stage_matrix_from_keys(dataset: Dict[str, np.ndarray], keys: Tuple[str, ...]) -> np.ndarray:
    missing = [key for key in keys if key not in dataset]
    if missing:
        available = ", ".join(sorted(dataset.keys()))
        raise SystemExit(
            f"Graph key(s) {missing} not in dataset. Available keys: {available}. "
...
def _stage_ids(stage: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
def compute_graph_potential(
    config: TrainConfig,
    dataset: Dict[str, np.ndarray],
) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    """Return Phi(s_i), Phi(s'_i) from a stage-transition graph.

    Goal nodes are chosen from max-reward transitions when a reward signal is
    present; otherwise from the stage(s) with the largest coordinate sum, which
    matches button-state puzzles where more pressed buttons means more progress.
    """
    keys = _resolve_graph_keys(config)
    stage = _stage_matrix_from_keys(dataset, keys)
    ids, reps = _stage_ids(stage)
    n_nodes = reps.shape[0]

    next_index = tmet.build_successor_index(dataset, boundary_tol=config.gps_boundary_tol)
    ...
    return phi, phi_next, info


def apply_graph_potential_shaping(config: TrainConfig, dataset: Dict[str, np.ndarray]) -> Dict[str, Any]:
    phi, phi_next, info = compute_graph_potential(config, dataset)
    rewards = np.asarray(dataset["rewards"], dtype=np.float32)
    shaping = config.gps_shaping_coef * (config.discount * phi_next - phi)
    dataset["rewards"] = (rewards + shaping).astype(np.float32)
    info.update({
        "shaping_coef": float(config.gps_shaping_coef),
        "shaping_mean": float(np.mean(shaping)),
        "shaping_std": float(np.std(shaping)),
        "shaping_min": float(np.min(shaping)),
        "shaping_max": float(np.max(shaping)),
    })
    print("Graph potential shaping | " + ", ".join(f"{k}={v}" for k, v in info.items()))
    return info
...
class GraphPotentialIQLJAX:
    ...
    def _build_train_step(self):
        ...
        def train_step(state: IQLState, batch: TensorBatch):
            ...
            # ----- Standard IQL expectile V update.
            ...
            # ----- Q update: same IQL backup, but rewards may already include
            # graph potential shaping in the replay buffer.
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="gps_iql_jax_training_progress",
    ...
    env, dataset, dataset_backend = load_env_and_dataset(
        ...
        add_info=(
            ...
            or config.gps_use_graph_shaping
        ),
    )
    ...
    if config.gps_use_graph_shaping:
        apply_graph_potential_shaping(config, dataset)
    ...
    print(
        f"iql_tau={config.iql_tau} | graph_shaping={config.gps_use_graph_shaping} | use_dcs={config.use_dcs} | "
