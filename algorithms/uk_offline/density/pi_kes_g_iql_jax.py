# pi_kes_g_iql_jax.py
#
# PI-KES-G-IQL: Prototype-Indexed Kernel Effective-Support IQL with State Gate
# (JAX/Flax).
#
# This variant preserves the original KES-G-IQL learning rule exactly, but
# replaces full-dataset kNN retrieval with:
#   prototype lookup -> candidate-pool retrieval -> exact top-k refinement.
#
...
ALGORITHM_NAME = "PI-KES-G-IQL"
ALGORITHM_FULL_NAME = "Prototype-Indexed Kernel Effective-Support IQL with State Gate"
...
class TrainConfig:
    ...
    # Prototypes are used only as a coarse inverted index. For each state,
    # members of the nearest prototype cells form a candidate pool, and the
    # exact top-k nearest dataset states are selected inside that pool.
    ...
    # Optional diagnostic: compare approximate neighbors against full-dataset
    # exact kNN on this many random anchors. Zero disables the diagnostic.
    bc_exact_recall_samples: int = 0

    # Force rebuilding the prototype-indexed profile cache.
...
def get_dataset_cache_id(env_name: str) -> str:
    """Return a canonical transition-dataset ID for cache sharing.
    ...
    task-dependent rewards / masks. Since the temporal encoder, coverage
    profile, and prototype-indexed profile use transition information rather
    than rewards, all ``taskN`` variants should reuse the same cache.
...
def _squared_distance_matrix(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    """Pairwise squared Euclidean distances with numerical clipping."""
...
def build_prototype_index_signature(
    ...
    """Signature for the raw prototype-indexed neighbor profile cache."""
    temporal_sig = None
    if config.dcs_metric_source == "temporal":
        temporal_sig = tmet.signature_hash(
            build_temporal_signature(
                config,
                observations_shape=tuple(metric_space.shape),
            )
        )

    oracle_keys = (
        list(resolve_oracle_keys(config) or ())
        if config.dcs_metric_source == "oracle"
        else None
    )
    ...
    return {
        ...
        "metric_dtype": str(np.asarray(metric_space).dtype),
        "actions_shape": tuple(actions.shape),
        "normalize": bool(config.normalize),
        "oracle_keys": oracle_keys,
        "oracle_standardize": (
            bool(config.dcs_oracle_standardize)
            if config.dcs_metric_source == "oracle"
            else None
        ),
...
def get_prototype_index_cache_path(
    ...
    root = Path(config.dcs_profile_path)
    env_name = _safe_path_name(get_dataset_cache_id(config.env))
    filename = f"prototype_indexed_kes_g_profile_{sig_hash}.npz"
...
def _fit_streaming_prototypes(
    ...
    """Fit state-space prototypes with streaming mini-batch k-means."""
    z = np.asarray(metric_space, dtype=np.float32)
    n = z.shape[0]
    if n < 2:
        raise ValueError("Prototype-indexed kNN requires at least two dataset states.")

    s = min(int(fit_samples), n)
    m = min(int(num_prototypes), n, s)
...
def _build_inverted_prototype_index(
    ...
    """Store cell members contiguously with offsets for fast lookup."""
...
def _gather_candidate_pool(
    ...
    """Gather nearby prototype cells, expanding until k non-self candidates exist."""
    ...
    for rank, proto in enumerate(prototype_order):
        ...
        # The anchor belongs to its nearest prototype, which is ranked first.
...
def _approximate_neighbors_from_prototype_index(
    ...
    """Retrieve nearby cells, then compute exact top-k inside the candidate pool."""
    z = np.asarray(metric_space, dtype=np.float32)
    n = z.shape[0]
    if n < 2:
        raise ValueError("Prototype-indexed kNN requires at least two dataset states.")
    ...
    for start in range(0, n, int(chunk_size)):
        ...
        for local_row, global_i in enumerate(range(start, end)):
            ...
            # Stable deterministic order: distance first, dataset index second.
...
def _compute_kes_statistics_from_neighbors(
    observations: np.ndarray,
    actions: np.ndarray,
    ...
    """Compute the same action-spread and displacement-dispersion statistics."""
    ...
    for start in range(0, n, int(chunk_size)):
        ...
        disp_dispersion[start:end] = np.clip(
            dispersion, 0.0, 1.0
        ).astype(np.float32)
...
def _prototype_index_exact_recall_diagnostic(
    ...
    """Optional recall@k check against full-dataset exact kNN."""
    n, k = approx_neighbors.shape
    sample_count = int(min(max(sample_count, 0), n))

    if sample_count <= 0:
        return {
            "exact_recall_samples": 0.0,
            "exact_recall_at_k": float("nan"),
        }
    ...
    for i in anchors:
        ...
        overlap = len(
            set(exact.tolist()) & set(approx_neighbors[i].tolist())
        )
...
def compute_prototype_indexed_profile(
    ...
    """Build raw KES-G profile with prototype retrieval plus local exact top-k."""
    ...
    n = observations.shape[0]
    if n < 2:
        raise ValueError("Prototype-indexed KES-G requires at least two transitions.")

    fit_samples = min(int(config.bc_prototype_fit_samples), n)
    n_prototypes = min(int(config.bc_num_prototypes), n, fit_samples)
    n_near = min(int(config.bc_nearest_prototypes), n_prototypes)
    k_eff = int(min(config.dcs_k, n - 1))

    print(
        f"Building prototype-indexed approximate KES-G profile: N={n}, "
        f"prototypes={n_prototypes}, initial_near_prototypes={n_near}, "
        f"k={k_eff}"
    )
    ...
    action_spread, disp_dispersion = _compute_kes_statistics_from_neighbors(
    ...
    print(
        ...
        f"nonempty_cells={positive_cell_counts.size}/{n_prototypes} | "
        f"mean_candidates={float(np.mean(candidate_count)):.2f} | "
        f"median_candidates={float(np.median(candidate_count)):.2f} | "
        f"p95_candidates={float(np.percentile(candidate_count, 95)):.2f} | "
        f"mean_prototypes_used={float(np.mean(prototypes_used)):.2f} | "
        f"expanded_frac={expanded_frac:.4f}"
    )

    if int(config.bc_exact_recall_samples) > 0:
        print(
            "Approximation diagnostic | "
            f"exact_recall_samples={int(recall_diag['exact_recall_samples'])} | "
            f"recall@{k_eff}={recall_diag['exact_recall_at_k']:.6f}"
        )
    ...
    return {
        ...
        "prototype_nearest_d2": nearest_d2.astype(np.float32),
        "candidate_count": candidate_count.astype(np.int32),
        "prototypes_used": prototypes_used.astype(np.int32),
        "prototype_count": np.asarray([n_prototypes], dtype=np.int32),
        "nonempty_prototype_count": np.asarray(
            [positive_cell_counts.size],
            dtype=np.int32,
        ),
        "mean_candidate_count": np.asarray(
            [np.mean(candidate_count)],
            dtype=np.float32,
        ),
        "exact_recall_samples": np.asarray(
            [recall_diag["exact_recall_samples"]],
            dtype=np.float32,
        ),
        "exact_recall_at_k": np.asarray(
            [recall_diag["exact_recall_at_k"]],
            dtype=np.float32,
        ),
    }
...
def load_or_compute_prototype_indexed_profile(
    ...
    """Load or build the prototype-indexed approximate KES-G profile."""
    if config.dcs_metric_source == "oracle":
        metric_space = build_oracle_metric_space(config, dataset)
        print("PI-KES-G metric space: oracle (ceiling test only).")
    elif config.dcs_metric_source == "temporal":
        metric_space = build_temporal_metric_space(
            config,
            dataset,
            device=device,
        )
        print("PI-KES-G metric space: learned temporal representation.")
    else:
        metric_space = np.asarray(
            dataset["observations"],
            dtype=np.float32,
        )
        print("PI-KES-G metric space: normalized observations.")

    cache_path = get_prototype_index_cache_path(
        config,
        metric_space,
        dataset["actions"],
    )
    signature = build_prototype_index_signature(
        config,
        metric_space,
        dataset["actions"],
    )
    ...
    force_recompute = bool(
        config.bc_force_recompute or config.dcs_force_recompute
    )
            ...
            with np.load(cache_path, allow_pickle=False) as cached:
                if str(cached["signature_json"].item()) == signature_json:
                    profile = {
                        ...
                        if key != "signature_json"
                    }
                    print(
                        f"Loaded prototype-indexed KES-G profile from: "
                        f"{cache_path}"
                    )
                    return profile
        except Exception as exc:
            print(
                f"Failed to load prototype-indexed cache {cache_path}: "
                f"{exc}; recomputing."
            )

    if force_recompute and cache_path is not None:
        print(
            "Ignoring existing prototype-indexed profile cache because "
            "recompute was requested: "
            f"{cache_path}"
        )

    profile = compute_prototype_indexed_profile(
        config,
        dataset,
        metric_space,
    )
    ...
    if cache_path is not None:
        ...
        print(f"Saved prototype-indexed KES-G profile to: {cache_path}")
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="pi_kes_g_iql_jax_training_progress",
    ...
    # ----- PI-KES-G support profile: prototype-indexed kNN + kernel weights --
    neighbor_indices = None
    neighbor_weights = None
    junction = None
    buffer_neighbor_k = config.dcs_k
    if config.use_dcs:
        profile = load_or_compute_prototype_indexed_profile(config, dataset, device=jax_device)
    ...
    if config.use_dcs:
        print(f"support weights: G_i*K_ij with gate_source={config.dcs_gate_source}; prototype-indexed approximate kNN; no dynamics gate, no neighbor Q/action pooling")
        if config.dcs_metric_source == "oracle":
            keys = resolve_oracle_keys(config)
            print(
            ...
            print(
                f"*** TEMPORAL METRIC: prototype-indexed approximate kNN built in a LEARNED temporal "
            ...
            print("metric source: observation (prototype-indexed approximate L2 kNN)")
    print("---------------------------------------")
