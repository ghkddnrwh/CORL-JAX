# soft_g_kes_iql_jax.py
#
# Soft-G-KES-IQL: Soft-Gated Kernel Effective-Support Implicit Q-Learning
# (JAX/Flax).
#
# This is the clean ablation that separates the two roles found in the
# KES-G/SCV experiments:
#   - K_ij estimates local effective support size from kNN distances.
#   - G_i is a state-level junction gate from density x diversity.
#
# Unlike KES-G-IQL, this algorithm does NOT put G_i inside the ESS weights
# w_ij = G_i K_ij, because a row-wise scalar cancels inside ESS whenever
# G_i > 0. Instead it computes the kernel effective-support confidence c_K(i)
# first, and then applies the soft gate multiplicatively:
#
#   K_ij    = exp(-(d_ij / h_i)^2)
#   N_eff^K = (sum_j K_ij)^2 / (sum_j K_ij^2 + eps)
#   c_K(i) = clip((N_eff^K(i) - 1) / (k - 1), 0, 1)
#   c_i    = (G_i * c_K(i))^p
#   tau_i  = tau_min + (tau_max - tau_min) * c_i
#
# The value loss remains in-sample IQL. Q backup and actor extraction are
# standard IQL/AWR. No neighbor Q/action pooling is used.
#
# This variant tests whether the continuous magnitude of G_i matters, rather
# than only the hard event G_i > 0.
...
ALGORITHM_NAME = "Soft-G-KES-IQL"
ALGORITHM_FULL_NAME = "Soft-Gated Kernel Effective-Support Implicit Q-Learning"
...
class TrainConfig:
    ...
    # ----- Soft-G-KES-IQL: soft-gated kernel effective-support value learning --------------
    # Neighbor Q/action pooling is not used. Kernel ESS gives c_K(s), while
    # junction gate G(s) softly scales the final support confidence c(s).
    ...
    # Legacy compatibility only. Soft-G-KES-IQL never pools neighbor Q-values or
...
def build_neighbor_weights(
    ...
    """Build kernel weights and a separate soft junction gate for Soft-G-KES-IQL.

    Returns:
        neighbor_indices (N, k) int32
        neighbor_weights (N, k) float32
            = K_ij = exp(-(d_ij / h_i)^2).  G_i is NOT multiplied into
              these weights, so its magnitude cannot cancel inside ESS.
        support_proxy (N,) float32
            = G_i = ramp-gate(J_i), stored in the existing auxiliary slot.

    The train step computes
        c_K(i) = clip((ESS(K_i) - 1) / (k - 1), 0, 1)
        c_i    = (G_i * c_K(i))^p
        tau_i  = tau_min + (tau_max - tau_min) * c_i.
    """
    ...
    kernel_active_edge_frac = float(np.mean(kernel > 1e-6))
    gate_active_row_frac = float(np.mean(gate > 1e-6))
    mean_kernel_mass = float(np.mean(np.sum(kernel, axis=1)))
    mean_kernel_weight = float(np.mean(kernel))

    print(
        "Soft-G-KES support profile | "
        f"c(s)=G_i*c_K(i) | mean G={float(np.mean(gate)):.4f} | "
        f"gate-active-row frac={gate_active_row_frac:.4f} | "
        f"mean kernel weight={mean_kernel_weight:.4f} | "
        f"mean kernel mass={mean_kernel_mass:.4f} | "
        f"kernel-active-edge frac={kernel_active_edge_frac:.4f}"
    )
    print(
        "Coverage profile | "
...
class KernelEffectiveSupportIQLJAX:
    ...
    def _build_train_step(self):
        ...
        def train_step(state: IQLState, batch: TensorBatch):
            ...
            neighbor_weights = jnp.clip(batch["neighbor_weights"], 0.0, 1.0)  # K_ij only
            state_gate = jnp.clip(jnp.squeeze(batch["junction"], axis=-1), 0.0, 1.0)  # G_i

            # Soft-G-KES support certificate.
            # First compute kernel-only effective support c_K(i), then apply
            # the continuous state gate G_i outside ESS so its magnitude is not
            # canceled by ESS scale invariance.
            ...
            kernel_conf = jnp.where(
                neighbor_mass > support_eps,
                jnp.clip((ess - 1.0) / denom, 0.0, 1.0),
                0.0,
            )
            support_conf_raw = jnp.clip(state_gate * kernel_conf, 0.0, 1.0)
            support_conf = support_conf_raw ** support_power
            ...
            log_dict = {
                ...
                # Soft-G-KES diagnostics
                "support_conf_mean": jnp.mean(support_conf),
                "support_conf_min": jnp.min(support_conf),
                "support_conf_max": jnp.max(support_conf),
                "support_conf_raw_mean": jnp.mean(support_conf_raw),
                "kernel_conf_mean": jnp.mean(kernel_conf),
                "kernel_conf_min": jnp.min(kernel_conf),
                "kernel_conf_max": jnp.max(kernel_conf),
                "state_gate_mean": jnp.mean(state_gate),
                "state_gate_min": jnp.min(state_gate),
                "state_gate_max": jnp.max(state_gate),
                ...
                "gate_active_frac": jnp.mean((state_gate > 0.0).astype(jnp.float32)),
                "neighbor_active_row_frac": jnp.mean(
                    (jnp.max(neighbor_weights, axis=1) > 0.0).astype(jnp.float32)
                ),
                "active_edge_frac": jnp.mean(
                    (neighbor_weights > 0.0).astype(jnp.float32)
                ),
                "neighbor_active_edge_frac": jnp.mean(
                    (neighbor_weights > 0.0).astype(jnp.float32)
                ),
            }
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="soft_g_kes_iql_jax_training_progress",
