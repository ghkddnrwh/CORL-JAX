...
ALGORITHM_NAME = "VGR-IQL"
ALGORITHM_FULL_NAME = "Value-Geometry Regularized Implicit Q-Learning"
...
class TrainConfig:
    ...
    # ----- VGR-IQL: value-geometry regularization -------------------------
    # Standard IQL V target is preserved. Neighbor graph is used only as an
    # auxiliary regularizer on V(s), never as a cross-state expectile target.
    vgr_smooth_coef: float = 0.01
    vgr_order_coef: float = 0.0
    vgr_order_margin: float = 0.0
    vgr_use_graph_order: bool = False
    vgr_graph_keys: Optional[str] = None
    vgr_normalize_potential: bool = True
    vgr_boundary_tol: float = 1e-4
    vgr_eps: float = 1e-8
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.vgr_smooth_coef >= 0.0
    assert config.vgr_order_coef >= 0.0
    assert config.vgr_order_margin >= 0.0
    assert config.vgr_boundary_tol >= 0.0
    assert config.vgr_eps > 0.0
...
# ---------------------------------------------------------------------------
# Graph potential helpers for VGR order regularization
# ---------------------------------------------------------------------------

def _resolve_vgr_graph_keys(config: TrainConfig) -> Tuple[str, ...]:
    if config.vgr_graph_keys:
        return tuple(x.strip() for x in config.vgr_graph_keys.split(",") if x.strip())
    keys = resolve_oracle_keys(config)
    if keys:
        return keys
    raise SystemExit(
        f"vgr_use_graph_order=True but no graph keys could be resolved for env "
        f"'{config.env}'. Pass --vgr_graph_keys, e.g. 'button_states'."
    )


def _vgr_stage_matrix_from_keys(dataset: Dict[str, np.ndarray], keys: Tuple[str, ...]) -> np.ndarray:
    missing = [key for key in keys if key not in dataset]
    if missing:
        available = ", ".join(sorted(dataset.keys()))
        raise SystemExit(
            f"VGR graph key(s) {missing} not in dataset. Available keys: {available}. "
            f"For OGBench, make sure add_info=True is used."
        )
    parts = []
    for key in keys:
...
def _vgr_stage_ids(stage: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    rounded = np.round(np.asarray(stage, dtype=np.float32), decimals=6)
    mapping: Dict[Tuple[float, ...], int] = {}
    ids = np.empty((rounded.shape[0],), dtype=np.int64)
    reps = []
    for i, row in enumerate(rounded):
        ...
        ids[i] = mapping[key]
    return ids, np.asarray(reps, dtype=np.float32)


def compute_vgr_graph_potential(config: TrainConfig, dataset: Dict[str, np.ndarray]) -> Tuple[np.ndarray, Dict[str, Any]]:
    keys = _resolve_vgr_graph_keys(config)
    stage = _vgr_stage_matrix_from_keys(dataset, keys)
    ids, reps = _vgr_stage_ids(stage)
    n_nodes = reps.shape[0]
    next_index = tmet.build_successor_index(dataset, boundary_tol=config.vgr_boundary_tol)
    valid_next = next_index >= 0
    next_ids = np.where(valid_next, ids[np.maximum(next_index, 0)], ids)
    reverse_adj = [[] for _ in range(n_nodes)]
    for u, v, ok in zip(ids, next_ids, valid_next):
        if ok and u != v:
            reverse_adj[int(v)].append(int(u))

    rewards = np.asarray(dataset.get("rewards", np.zeros_like(ids, dtype=np.float32))).reshape(-1)
    if rewards.size == ids.size and float(np.max(rewards)) > float(np.min(rewards)):
        max_r = float(np.max(rewards))
        goal_nodes = set(int(next_ids[i]) for i in np.where(rewards >= max_r)[0])
        goal_strategy = "max_reward_next_stage"
    else:
        sums = reps.sum(axis=1)
        max_sum = float(np.max(sums)) if sums.size else 0.0
        goal_nodes = set(int(i) for i in np.where(sums >= max_sum)[0])
        goal_strategy = "max_stage_sum"
    ...
    while q:
        ...
        for u in reverse_adj[v]:
            if dist[u] == inf:
                ...
                q.append(u)
    finite = dist < inf
    max_finite = int(np.max(dist[finite])) if np.any(finite) else 1
    dist_filled = dist.copy()
    dist_filled[~finite] = max_finite + 1
    # Larger potential means closer to goal.
    phi_nodes = -dist_filled.astype(np.float32)
    if config.vgr_normalize_potential:
        denom = max(float(np.max(dist_filled)), 1.0)
        phi_nodes = phi_nodes / denom
    phi = phi_nodes[ids].astype(np.float32)
    info = {
        "graph_keys": list(keys),
        "num_stage_nodes": int(n_nodes),
        "num_goal_nodes": int(len(goal_nodes)),
        "goal_strategy": goal_strategy,
        "reachable_node_frac": float(np.mean(finite)) if n_nodes else 0.0,
        "phi_min": float(np.min(phi_nodes)) if n_nodes else 0.0,
        "phi_max": float(np.max(phi_nodes)) if n_nodes else 0.0,
    }
    print("VGR graph potential | " + ", ".join(f"{k}={v}" for k, v in info.items()))
    return phi, info
...
class ReplayBuffer:
    ...
    def __init__(
        ...
        self._stage_potential = np.zeros((buffer_size, 1), dtype=np.float32)
        self._device = device
    ...
    def load_d4rl_dataset(
        ...
        stage_potential: Optional[np.ndarray] = None,
    ):
        if self._size != 0:
        ...
        if stage_potential is None:
            stage_potential = np.zeros((n_transitions,), dtype=np.float32)
        ...
        stage_potential = np.asarray(stage_potential, dtype=np.float32).reshape(-1)
        ...
        if stage_potential.shape[0] != n_transitions:
            raise ValueError("stage_potential must have one scalar per transition")
        ...
        self._stage_potential[:n_transitions] = stage_potential[:, None]
    ...
    def sample(self, batch_size: int) -> TensorBatch:
        ...
        batch = {
            ...
            "stage_potential": self._stage_potential[indices],  # (B, 1)
            "neighbor_stage_potential": self._stage_potential[nbr_idx],  # (B, k, 1)
        }
...
class ValueGeometryIQLJAX:
    ...
    def __init__(
        ...
        self.vgr_smooth_coef = float(vgr_smooth_coef)
        self.vgr_order_coef = float(vgr_order_coef)
        self.vgr_order_margin = float(vgr_order_margin)
        self.vgr_eps = float(vgr_eps)
    ...
    def _build_train_step(self):
        ...
        smooth_coef = self.vgr_smooth_coef
        order_coef = self.vgr_order_coef
        order_margin = self.vgr_order_margin
        eps = self.vgr_eps
        ...
        def train_step(state: IQLState, batch: TensorBatch):
            ...
            neighbor_observations = batch["neighbor_observations"]
            neighbor_weights = jnp.clip(batch["neighbor_weights"], 0.0, 1.0)
            stage_potential = jnp.squeeze(batch.get("stage_potential", jnp.zeros((observations.shape[0], 1))), axis=-1)
            neighbor_stage_potential = jnp.squeeze(
                batch.get("neighbor_stage_potential", jnp.zeros((observations.shape[0], neighbor_weights.shape[1], 1))),
                axis=-1,
            )
            ...
            # ----- Standard IQL expectile V update + geometry regularizers.
            def v_loss_fn(v_params):
                ...
                base_value_loss = jnp.mean(expectile_weight * diff ** 2)

                bsz, k = neighbor_observations.shape[0], neighbor_observations.shape[1]
                flat_nbr = neighbor_observations.reshape(bsz * k, -1)
                nbr_v = v_apply({"params": v_params}, flat_nbr).reshape(bsz, k)
                row_mass = jnp.sum(neighbor_weights, axis=1)
                smooth_row = jnp.sum(neighbor_weights * (v[:, None] - nbr_v) ** 2, axis=1) / (row_mass + eps)
                smooth_loss = jnp.mean(jnp.where(row_mass > eps, smooth_row, 0.0))

                better_neighbor = (neighbor_stage_potential > stage_potential[:, None] + 1e-6).astype(jnp.float32)
                order_w = neighbor_weights * better_neighbor
                order_violation = jax.nn.relu(v[:, None] + order_margin - nbr_v)
                order_numer = jnp.sum(order_w * order_violation ** 2)
                order_denom = jnp.sum(order_w) + eps
                order_loss = order_numer / order_denom
                total_value_loss = base_value_loss + smooth_coef * smooth_loss + order_coef * order_loss
                return total_value_loss, (v, base_value_loss, smooth_loss, order_loss, nbr_v)

            (value_loss, (v, base_value_loss, smooth_loss, order_loss, nbr_v)), v_grads = jax.value_and_grad(
            ...
            log_dict = {
                ...
                "base_value_loss": base_value_loss,
                "smooth_value_loss": smooth_loss,
                "order_value_loss": order_loss,
                "v_mean": jnp.mean(v),
                "neighbor_v_mean": jnp.mean(nbr_v),
                ...
                "active_edge_frac": jnp.mean((neighbor_weights > 0.0).astype(jnp.float32)),
                "stage_potential_mean": jnp.mean(stage_potential),
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="vgr_iql_jax_training_progress",
    ...
    env, dataset, dataset_backend = load_env_and_dataset(
        config.env,
        add_info=(
            (config.use_dcs and config.dcs_metric_source == "oracle")
            or config.vgr_use_graph_order
        ),
    )
    ...
    # ----- Optional graph potential for VGR order regularization ------------
    stage_potential = None
    if config.vgr_use_graph_order:
        stage_potential, _ = compute_vgr_graph_potential(config, dataset)
    ...
    print(
        f"iql_tau={config.iql_tau} | VGR smooth={config.vgr_smooth_coef} order={config.vgr_order_coef if config.vgr_use_graph_order else 0.0} | use_dcs={config.use_dcs} | "
    ...
    trainer = ValueGeometryIQLJAX(
        ...
        vgr_smooth_coef=config.vgr_smooth_coef,
        vgr_order_coef=config.vgr_order_coef if config.vgr_use_graph_order else 0.0,
        vgr_order_margin=config.vgr_order_margin,
        vgr_eps=config.vgr_eps,
