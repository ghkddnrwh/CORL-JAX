# kes_iql_jax.py
#
# KES-IQL: Kernel Effective-Support Implicit Q-Learning (JAX/Flax).
#
# This is the simplified, paper-friendly SCV-IQL variant discussed in the
# analysis: it removes the junction gate J(s), the density/diversity gate, the
# dynamics-consistency gate, and all neighbor Q/action pooling from the main
# algorithm. The kNN graph is used only to estimate a local effective support
# size with a distance kernel.
#
# For each dataset state s_i:
#   1) Build kNN neighbors N_k(i) in a chosen metric space phi(s): observation,
#      oracle ceiling, or learned temporal representation.
#   2) Convert neighbor distances to kernel weights
#          K_ij = exp(-(d_ij / h_i)^2).
#   3) Estimate effective support size
#          N_eff(i) = (sum_j K_ij)^2 / (sum_j K_ij^2 + eps).
#   4) Convert it to confidence
#          c_i = clip((N_eff(i) - 1) / (k - 1), 0, 1)^p.
#   5) Interpolate IQL's value expectile
#          tau_i = tau_min + (tau_max - tau_min) * c_i.
#
# The value loss is still in-sample IQL:
#          L_V = E_i[ |tau_i - 1(Q(s_i,a_i)-V(s_i)<0)| (Q(s_i,a_i)-V(s_i))^2 ].
# Q-learning backup and actor extraction are standard IQL/AWR. No neighbor
# action copying and no cross-state Q pooling are used.
#
# Legacy names such as use_dcs, dcs_k, dcs_metric_source, and dcs_profile_path
# are kept for compatibility with the existing scripts and cache plumbing, but
# in KES-IQL the support weights are always kernel-only.
...
ALGORITHM_NAME = "KES-IQL"
ALGORITHM_FULL_NAME = "Kernel Effective-Support Implicit Q-Learning"
...
class TrainConfig:
    ...
    # ----- KES-IQL: kernel effective-support value learning --------------
    # Neighbor Q/action pooling is not used. Kernel weights over the kNN graph
...
def build_neighbor_weights(
    ...
    """Build kernel-only support weights for KES-IQL.

    Returns:
        neighbor_indices (N, k) int32
        neighbor_weights (N, k) float32
            = K_ij = exp(-(d_ij / h_i)^2). No junction gate G_i and no
              dynamics gate g_ij are applied in the main KES-IQL algorithm.
        support_proxy    (N,) float32
            logging-only placeholder: per-state mean kernel weight.

    Important: N_k(i) has already been selected by coverage_profile in the
    chosen metric space. This function only turns those distances into K_ij.
    """
    ...
    # Logging-only proxy, kept in the ReplayBuffer's existing "junction" slot
    # for backward compatibility with the original code path.
    support_proxy = np.mean(kernel, axis=1).astype(np.float32)

    active_edge_frac = float(np.mean(kernel > 1e-6))
    mean_mass = float(np.mean(np.sum(kernel, axis=1)))
    mean_weight = float(np.mean(kernel))
    print(
        "KES support profile | "
        f"kernel-only weights | mean edge weight={mean_weight:.4f} | "
        f"mean per-state mass={mean_mass:.4f} | "
        f"active-edge frac={active_edge_frac:.4f}"
    )
    print(
    ...
    if zero_median_row_frac > 0.5:
        print(
            ...
            "in this metric space. In KES-IQL this means the kernel ESS will be "
            "close to k for many states, so tau(s) may approach scv_tau_max."
        )
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="kes_iql_jax_training_progress",
    ...
    print(
        f"kes_tau=[{config.scv_tau_min}, {config.scv_tau_max}] | use_support_graph={config.use_dcs} | "
