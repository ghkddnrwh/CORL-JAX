...
ALGORITHM_NAME = "SDG-IQL"
ALGORITHM_FULL_NAME = "State-Density Guided Implicit Q-Learning"
...
class TrainConfig:
    ...
    # SDG-IQL: State-Density Guided IQL
    # Main idea:
    #   - estimate marginal state density confidence c(s) once from the offline dataset
    #   - keep the critic expectile stable by default (fixed iql_tau)
    #   - use c(s) for replay rebalancing and actor extraction, not directly as optimism
    ...
    # precomputed c(s) array used by SDG-IQL so repeated runs can skip kNN.
    ...
    # Backward-compatible DC-IQL ablations. Keep False for the new SDG-IQL default.
    # If True, value expectile becomes tau(s)=dc_tau_min+(dc_tau_max-dc_tau_min)*c(s).
    dc_use_adaptive_tau: bool = False
    dc_tau_min: float = 0.7
    dc_tau_max: float = 0.9
    dc_vicinal_lambda: float = 0.0
    dc_vicinal_noise_std: float = 0.01

    # Density-guided replay sampling.
    # With c(s)=1 for dense states and c(s)=0 for sparse states,
    # weight(s)=(1-c(s)+eps)^alpha softly upweights low-density regions.
    sdg_use_density_sampling: bool = True
    sdg_sampling_alpha: float = 0.5
    sdg_sampling_eps: float = 0.05
    sdg_return_eta: float = 0.0  # optional RTG relevance; 0 disables it

    # Density-guided actor extraction.
    # The critic remains fixed-tau IQL; only the AWR/IQL actor weight cap is state-wise.
    sdg_use_density_actor: bool = True
    sdg_actor_clip_min: float = 10.0
    sdg_actor_clip_max: float = EXP_ADV_MAX
    sdg_actor_density_power: float = 1.0
    sdg_actor_return_mix: float = 0.0  # optional mix with normalized RTG relevance in [0,1]
    vf_lr: float = 3e-4
    qf_lr: float = 3e-4
    actor_lr: float = 3e-4
    ...
    group: str = "SDG-IQL-JAX"
    name: str = "SDG-IQL-JAX"
    log_wandb: bool = True
    log_every: int = 500
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.sdg_sampling_alpha >= 0.0
    assert config.sdg_sampling_eps > 0.0
    assert config.sdg_return_eta >= 0.0
    assert config.sdg_actor_clip_min > 0.0
    assert config.sdg_actor_clip_max >= config.sdg_actor_clip_min
    assert config.sdg_actor_density_power >= 0.0
    assert config.sdg_actor_return_mix >= 0.0 and config.sdg_actor_return_mix <= 1.0
...
def compute_state_density_confidence(
    states: np.ndarray,
    k: int = 10,
    ...
    """Return c(s) in [0, 1], where larger means denser local state coverage.

    This is a one-time preprocessing step. It avoids storing per-state neighbor
    lists. We only keep one scalar confidence for each transition.

    Implementation detail:
      - Build a KD-tree on a capped subsample of normalized states.
      - Query the distance to the k-th nearest reference state in chunks.
      - Convert smaller kNN radii into larger confidence by percentile scaling.

    For very large/high-dimensional datasets, this is still much cheaper than
    training-time neighborhood losses because the output is only N scalars.
    """
    ...
    # Prefer scipy's cKDTree; fall back to a random-projection norm heuristic if
    # scipy.spatial is unavailable in a minimal environment.
    try:
        ...
        # If querying the same set used to build the tree, k=1 is self-distance.
        query_k = int(min(max(k, 1), ref_size))
        radii = np.empty((n,), dtype=np.float32)
        for start in range(0, n, int(chunk_size)):
            end = min(start + int(chunk_size), n)
            distances, _ = tree.query(states[start:end], k=query_k, workers=-1)
            if query_k == 1:
                kth = distances
            else:
                kth = distances[:, -1]
            radii[start:end] = np.asarray(kth, dtype=np.float32)
    except Exception as exc:
        print(f"Density confidence: cKDTree unavailable ({exc}). Falling back to norm-based proxy.")
        centered = states - np.mean(states, axis=0, keepdims=True)
        radii = np.linalg.norm(centered, axis=-1).astype(np.float32)

    lo = np.percentile(radii, percentile_low)
    hi = np.percentile(radii, percentile_high)
    if not np.isfinite(lo) or not np.isfinite(hi) or hi <= lo:
        confidence = np.ones_like(radii, dtype=np.float32)
    else:
...
def compute_normalized_return_to_go(
    dataset: Dict[str, np.ndarray],
    discount: float,
    percentile_low: float = 5.0,
    percentile_high: float = 95.0,
) -> np.ndarray:
    """Return a normalized return-to-go score g(s) in [0, 1] for every transition.

    This is optional. It is useful when low-density states include sparse but
    important success bottlenecks. In that case, using density alone may suppress
    rare high-return states. The returned score can be mixed into sampling or
    actor reliability.
    """
    rewards = np.asarray(dataset["rewards"], dtype=np.float32).reshape(-1)
    if "masks" in dataset:
        continuation = np.asarray(dataset["masks"], dtype=np.float32).reshape(-1)
    else:
    ...
    for idx in range(len(rewards) - 1, -1, -1):
        running = float(rewards[idx]) + float(discount) * running * float(continuation[idx])
        rtg[idx] = running

    lo = np.percentile(rtg, percentile_low)
    hi = np.percentile(rtg, percentile_high)
    if not np.isfinite(lo) or not np.isfinite(hi) or hi <= lo:
        scores = np.zeros_like(rtg, dtype=np.float32)
    else:
        scores = np.clip((rtg - lo) / (hi - lo), 0.0, 1.0).astype(np.float32)
    return scores.reshape(-1, 1)


def build_sdg_sampling_weights(
    density_confidence: Optional[np.ndarray],
    rtg_scores: Optional[np.ndarray] = None,
    alpha: float = 0.5,
    eps: float = 0.05,
    return_eta: float = 0.0,
) -> Optional[np.ndarray]:
    """Build replay sampling probabilities from state density confidence.

    c(s)=1 means high-density, c(s)=0 means low-density. The inverse-density
    factor softly rebalances minibatches toward low-density states without
    changing Bellman targets. return_eta optionally protects high-return rare
    states by multiplying weights by exp(return_eta * normalized_RTG).
    """
    if density_confidence is None or alpha == 0.0:
        if return_eta <= 0.0 or rtg_scores is None:
            return None
        weights = np.exp(return_eta * np.asarray(rtg_scores, dtype=np.float32).reshape(-1))
    else:
        c = np.clip(np.asarray(density_confidence, dtype=np.float32).reshape(-1), 0.0, 1.0)
        weights = np.power(1.0 - c + float(eps), float(alpha))
        if return_eta > 0.0 and rtg_scores is not None:
            g = np.clip(np.asarray(rtg_scores, dtype=np.float32).reshape(-1), 0.0, 1.0)
            weights = weights * np.exp(float(return_eta) * g)
...
def load_or_compute_density_confidence(
    ...
    print("Computing one-time state-density confidence for SDG-IQL...")
...
class ReplayBuffer:
    def __init__(
        ...
        self._coverages = np.ones((buffer_size, 1), dtype=np.float32)
        self._rtg_scores = np.zeros((buffer_size, 1), dtype=np.float32)
    ...
    def load_d4rl_dataset(
        ...
        if coverage is not None:
            ...
            self._coverages[:n_transitions] = coverage

        if rtg_scores is not None:
            rtg_scores = np.asarray(rtg_scores, dtype=np.float32).reshape(-1, 1)
            if rtg_scores.shape[0] != n_transitions:
                raise ValueError("rtg_scores must have one scalar per transition")
            self._rtg_scores[:n_transitions] = rtg_scores

        if sampling_weights is not None:
            sampling_weights = np.asarray(sampling_weights, dtype=np.float64).reshape(-1)
            if sampling_weights.shape[0] != n_transitions:
                raise ValueError("sampling_weights must have one scalar per transition")
            sampling_weights = np.maximum(sampling_weights, 1e-12)
            self._sampling_weights = sampling_weights / np.sum(sampling_weights)
        ...
        if self._sampling_weights is not None:
            print(
                "SDG replay sampling enabled: "
                f"min_p={float(np.min(self._sampling_weights)):.3e}, "
                f"max_p={float(np.max(self._sampling_weights)):.3e}"
            )

    def sample(self, batch_size: int) -> TensorBatch:
        max_index = min(self._size, self._pointer)
        if self._sampling_weights is None:
            indices = np.random.randint(0, max_index, size=batch_size)
        else:
            indices = np.random.choice(max_index, size=batch_size, replace=True, p=self._sampling_weights[:max_index])
        batch = {
            ...
            "coverages": self._coverages[indices],
            "rtg_scores": self._rtg_scores[indices],
        }
...
class IQLJAX:
    """State-Density Guided Implicit Q-Learning in JAX/Flax.
    ...
    def __init__(
        ...
        sdg_actor_clip_max: float = EXP_ADV_MAX,
        sdg_actor_density_power: float = 1.0,
        sdg_actor_return_mix: float = 0.0,
        actor_dropout: Optional[float] = None,
        hidden_dim: int = 256,
        n_hidden: int = 2,
        seed: int = 0,
        ...
        self.dc_use_adaptive_tau = dc_use_adaptive_tau
        ...
        self.sdg_use_density_actor = sdg_use_density_actor
        self.sdg_actor_clip_min = sdg_actor_clip_min
        self.sdg_actor_clip_max = sdg_actor_clip_max
        self.sdg_actor_density_power = sdg_actor_density_power
        self.sdg_actor_return_mix = sdg_actor_return_mix
    ...
    def _build_train_step(self):
        ...
        dc_use_adaptive_tau = self.dc_use_adaptive_tau
        ...
        sdg_use_density_actor = self.sdg_use_density_actor
        sdg_actor_clip_min = self.sdg_actor_clip_min
        sdg_actor_clip_max = self.sdg_actor_clip_max
        sdg_actor_density_power = self.sdg_actor_density_power
        sdg_actor_return_mix = self.sdg_actor_return_mix
        ...
        def train_step(state: IQLState, batch: TensorBatch):
            ...
            coverages = jnp.squeeze(batch.get("coverages", jnp.ones_like(batch["dones"])), axis=-1)
            rtg_scores = jnp.squeeze(batch.get("rtg_scores", jnp.zeros_like(batch["dones"])), axis=-1)
            state_conf = jnp.clip(coverages, 0.0, 1.0)
            rtg_conf = jnp.clip(rtg_scores, 0.0, 1.0)
            batch_iql_tau = jnp.where(
                dc_use_adaptive_tau,
                dc_tau_min + (dc_tau_max - dc_tau_min) * state_conf,
                jnp.ones_like(state_conf) * iql_tau,
            )
            ...
            exp_adv_raw = jnp.exp(beta * jax.lax.stop_gradient(adv))
            actor_reliability = jnp.clip(
                (1.0 - sdg_actor_return_mix) * jnp.power(state_conf, sdg_actor_density_power)
                + sdg_actor_return_mix * rtg_conf,
                0.0,
                1.0,
            )
            density_actor_cap = sdg_actor_clip_min + (sdg_actor_clip_max - sdg_actor_clip_min) * actor_reliability
            exp_adv = jnp.where(
                sdg_use_density_actor,
                jnp.minimum(exp_adv_raw, density_actor_cap),
                jnp.minimum(exp_adv_raw, EXP_ADV_MAX),
            )

            # SDG-IQL value update:
            #   - by default uses fixed iql_tau for critic stability
            #   - optional dc_use_adaptive_tau=True recovers the old DC-IQL ablation
            #   - optional vicinal loss is kept for ablation, but default is 0.0
            ...
            def v_loss_fn(v_params):
                ...
                value_weight = jnp.abs(batch_iql_tau - (value_adv < 0.0).astype(jnp.float32))
                expectile_loss = jnp.mean(value_weight * value_adv ** 2)

                noise = jax.random.normal(v_noise_key, observations.shape, dtype=observations.dtype)
                noise = noise * dc_vicinal_noise_std * state_conf[:, None]
                ...
                vicinal_loss = jnp.mean(state_conf * (v_perturbed - v_anchor) ** 2)
                value_loss = expectile_loss + dc_vicinal_lambda * vicinal_loss
                return value_loss, (v, expectile_loss, vicinal_loss)

            (value_loss, (v, expectile_loss, vicinal_loss)), v_grads = jax.value_and_grad(v_loss_fn, has_aux=True)(state.v_params)
            ...
            log_dict = {
                ...
                "value_vicinal_loss": vicinal_loss,
                "density_conf_mean": jnp.mean(state_conf),
                "density_conf_min": jnp.min(state_conf),
                "density_conf_max": jnp.max(state_conf),
                "iql_tau_mean": jnp.mean(batch_iql_tau),
                "iql_tau_min": jnp.min(batch_iql_tau),
                "iql_tau_max": jnp.max(batch_iql_tau),
                "rtg_conf_mean": jnp.mean(rtg_conf),
                "actor_reliability_mean": jnp.mean(actor_reliability),
                "actor_weight_cap_mean": jnp.mean(density_actor_cap),
                "actor_weight_cap_min": jnp.min(density_actor_cap),
                "actor_weight_cap_max": jnp.max(density_actor_cap),
                "exp_adv_raw_mean": jnp.mean(exp_adv_raw),
                "v_mean": jnp.mean(v),
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="sdg_iql_jax_training_progress",
    ...
    if config.use_density_calibration:
        ...
        print(
            f"Density confidence: mean={float(np.mean(density_confidence)):.3f}, "
            f"min={float(np.min(density_confidence)):.3f}, max={float(np.max(density_confidence)):.3f}; "
            f"adaptive tau ablation values: mean={float(np.mean(tau_values)):.3f}, "
            f"min={float(np.min(tau_values)):.3f}, max={float(np.max(tau_values)):.3f}; "
            f"dc_use_adaptive_tau={config.dc_use_adaptive_tau}"
        )

        rtg_scores = compute_normalized_return_to_go(
            dataset=dataset,
        ...
        print(
            f"RTG relevance score: mean={float(np.mean(rtg_scores)):.3f}, "
            f"min={float(np.min(rtg_scores)):.3f}, max={float(np.max(rtg_scores)):.3f}"
        )

        if config.sdg_use_density_sampling:
            sampling_weights = build_sdg_sampling_weights(
                density_confidence=density_confidence,
                rtg_scores=rtg_scores,
                alpha=config.sdg_sampling_alpha,
                eps=config.sdg_sampling_eps,
                return_eta=config.sdg_return_eta,
            )
            if sampling_weights is not None:
                print(
                    f"SDG sampling weights: mean={float(np.mean(sampling_weights)):.3e}, "
                    f"min={float(np.min(sampling_weights)):.3e}, "
                    f"max={float(np.max(sampling_weights)):.3e}"
                )
    ...
    trainer = IQLJAX(
        ...
        dc_use_adaptive_tau=config.dc_use_adaptive_tau if config.use_density_calibration else False,
        sdg_use_density_actor=config.sdg_use_density_actor if config.use_density_calibration else False,
        sdg_actor_clip_min=config.sdg_actor_clip_min,
        sdg_actor_clip_max=config.sdg_actor_clip_max,
        sdg_actor_density_power=config.sdg_actor_density_power,
        sdg_actor_return_mix=config.sdg_actor_return_mix,
