...
ALGORITHM_NAME = "DC-IQL"
ALGORITHM_FULL_NAME = "Density-Calibrated Implicit Q-Learning"
...
class TrainConfig:
    ...
    # DC-IQL: Density-Calibrated IQL
    # 1) state-wise expectile: tau(s)=dc_tau_min+(dc_tau_max-dc_tau_min)*c(s)
    # 2) optional density-gated vicinal consistency on V(s)
...
class ReplayBuffer:
    ...
    def load_d4rl_dataset(self, data: Dict[str, np.ndarray], coverage: Optional[np.ndarray] = None):
        if self._size != 0:
...
class IQLJAX:
    """Density-Calibrated Implicit Q-Learning in JAX/Flax.
    ...
    def _build_train_step(self):
        ...
        def train_step(state: IQLState, batch: TensorBatch):
            ...
            batch_iql_tau = dc_tau_min + (dc_tau_max - dc_tau_min) * state_conf
            ...
            # DC-IQL value update:
            #   - replace the global expectile level with tau(s), calibrated by
            #     one-time state-density confidence c(s)
            #   - add a cheap density-gated vicinal consistency term on V(s)
            #     to make IQL's implicit local generalization explicit
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="density_calibrated_iql_jax_training_progress",
    ...
    replay_buffer.load_d4rl_dataset(dataset, coverage=density_confidence)
