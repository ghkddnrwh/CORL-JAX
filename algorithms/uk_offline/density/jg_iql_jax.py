...
#
# Ablation variant for support-certified value learning.
#
# This variant removes kernel effective-support ESS from the value-control
# path and uses the state-level junction gate directly as the confidence:
#
#   density_conf_i = percentile_confidence(knn_radius_i, invert=True)
#   diversity_conf_i = action / displacement / product diversity confidence
#   J_i = density_conf_i * diversity_conf_i
#   G_i = ramp_gate(J_i) in [0, 1]
#   c_i = G_i^p
#   tau_i = tau_min + (tau_max - tau_min) * c_i
#
# Thus JG-IQL answers the ablation question:
#   Is the state-level density/diversity junction gate itself enough to decide
#   where IQL should be optimistic?
#
...
# Q-value pooling, no neighbor action copying, no kernel ESS, and no dynamics
# gate are used in the value-control path.
#
import copy
import json
import os
...
ALGORITHM_NAME = "JG-IQL"
ALGORITHM_FULL_NAME = "Junction-Gated Implicit Q-Learning"
...
class TrainConfig:
    ...
    # ----- JG-IQL: state-gated kernel effective-support value learning --------------
...
def build_neighbor_weights(
    ...
    """Build state-level junction-gate inputs for JG-IQL.

    Returns:
        neighbor_indices (N, k) int32
            Kept only for ReplayBuffer compatibility. JG-IQL does not use
            neighbor observations/actions in the train step.
        neighbor_weights (N, k) float32
            Legacy diagnostic tensor. Each row repeats G_i across k entries so
            old active-row/active-edge logging remains meaningful. It is NOT
            used to compute tau(s).
        gate             (N,) float32
            The actual support confidence base G_i used by the train step:
                c_i = G_i ** scv_support_power.

    Important: the kNN graph is still needed to compute density/diversity
    statistics, but JG-IQL does not use K_ij, ESS, or g_ij in the value-control
    path.
    """
    ...
    # active-edge/row logging interpretable without adding a new ReplayBuffer
    # field.
    k_eff = int(profile["neighbor_indices"].shape[1])
    neighbor_weights = np.repeat(gate[:, None], k_eff, axis=1).astype(np.float32)
    ...
    active_row_frac = float(np.mean(gate > 1e-6))
    ...
    print(
        f"kNN diagnostics | zero-distance edges={zero_edge_frac:.3f} | "
