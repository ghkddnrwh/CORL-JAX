# relational_branching_gated_iql_jax.py
#
# RBG-IQL: Relational Branching-Gated Implicit Q-Learning (JAX/Flax).
#
# Practical graph-free variant motivated by three empirical observations:
#   1) JG-IQL can match KES-G-IQL, suggesting kernel ESS is not always essential.
#   2) product_diversity can outperform density_x_product, suggesting that
#      local action/continuation diversity is more relevant than visitation density.
#   3) point-wise moment prediction can fail to recover actual relational diversity.
#
# RBG-IQL therefore keeps real relational comparisons between samples but removes
# the full-dataset kNN graph. After learning (or loading) a temporal representation
#     z_i = phi(s_i),
# it repeatedly draws an anchor mini-batch and a stochastic reference queue and
# forms a soft temporal neighborhood
#     alpha_ij = softmax(sim(z_i, z_j) / T_rel).
#
# From actual reference actions a_j and normalized displacement directions u_j,
# it computes three relational teacher targets:
#     A_i : local action diversity,
#     P_i : local continuation/displacement diversity,
#     R_i : normalized action-outcome coupling.
#
# A lightweight branch-statistics head h_psi(z) is distilled from these stochastic
# relational targets. After distillation, one forward pass over the dataset gives
# per-state scores; no N x k graph, neighbor indices, neighbor distances, density
# estimate, kernel ESS, or neighbor pooling is stored or used during RL training.
#
# Gate modes:
#     action             : J_i = A_i
...
#     coupling           : J_i = R_i
#     coupled_product    : J_i = sqrt(A_i * P_i) * R_i^eta
#
# Then
#     G_i   = ramp_gate(J_i),
#     c_i   = G_i^p,
...
ALGORITHM_NAME = "RBG-IQL"
ALGORITHM_FULL_NAME = "Relational Branching-Gated Implicit Q-Learning"
...
class TrainConfig:
    ...
    # ----- RBG-IQL: graph-free relational branching gate -------------------
    ...
    # Which distilled relational signal controls the gate.
    #   action            : J_i = A_i
    #   displacement      : J_i = P_i
    #   product_diversity : J_i = sqrt(A_i * P_i)
    #   coupling          : J_i = R_i
    #   coupled_product   : J_i = sqrt(A_i * P_i) * R_i ** branch_coupling_power
    dcs_gate_source: str = "coupled_product"

    # Backward-compatible alias for older YAML files.
    dcs_diversity_mode: str = "product"

    # Percentile scaling of predicted relational statistics into [0, 1].
    dcs_percentile_low: float = 5.0
    dcs_percentile_high: float = 95.0

    # Ramp gate over the selected relational signal J.
    dcs_gate_low_percentile: float = 60.0
    dcs_gate_high_percentile: float = 95.0

    # Cache root for temporal embeddings and relational branching profiles.
    ...
    # No explicit full-dataset kNN graph is ever constructed.
    ...
    # Keep representation seed independent of RL seed so the same embedding
    # cache can be shared by multiple policy-training seeds.
    dcs_temporal_seed: int = 0

    # ----- Stochastic relational teacher + branch-statistics distillation --
    # Each update samples B anchors and R reference transitions. The references
    # form a stochastic queue for that update; soft temporal similarities define
    # relational neighborhood weights. Nothing is stored as a persistent graph.
    branch_hidden_dim: int = 128
    branch_layers: int = 2
    branch_steps: int = 30_000
    branch_anchor_batch_size: int = 256
    branch_reference_batch_size: int = 1024
    branch_lr: float = 3e-4
    branch_weight_decay: float = 1e-5
    branch_relation_temperature: float = 0.05
    branch_action_coef: float = 1.0
    branch_displacement_coef: float = 1.0
    branch_coupling_coef: float = 1.0
    # Product score uses R^eta when dcs_gate_source="coupled_product".
    branch_coupling_power: float = 1.0
    # Global scale for normalizing the teacher's action-spread target. The base
    # scale is sqrt(sum_d Var_dataset[a_d]); this multiplier controls softness.
    branch_action_scale_multiplier: float = 1.0
    branch_inference_chunk_size: int = 65_536
    branch_seed: int = 0
    branch_force_recompute: bool = False
    # -----------------------------------------------------------------------
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.dcs_gate_source in (
        "action", "displacement", "product_diversity", "coupling", "coupled_product"
    )
    ...
    assert config.branch_hidden_dim >= 1
    assert config.branch_layers >= 1
    assert config.branch_steps >= 1
    assert config.branch_anchor_batch_size >= 1
    assert config.branch_reference_batch_size >= 2
    assert config.branch_lr > 0.0
    assert config.branch_weight_decay >= 0.0
    assert config.branch_relation_temperature > 0.0
    assert config.branch_action_coef >= 0.0
    assert config.branch_displacement_coef >= 0.0
    assert config.branch_coupling_coef >= 0.0
    assert config.branch_coupling_power >= 0.0
    assert config.branch_action_scale_multiplier > 0.0
    assert config.branch_inference_chunk_size >= 1
...
def apply_env_hyperparams(config: TrainConfig) -> TrainConfig:
    ...
    # RBG-IQL deliberately keeps its own default gate source (coupled_product).
    # Older YAML files may still contain dcs_diversity_mode; that legacy field is
    # accepted for compatibility but does not silently override dcs_gate_source.
...
# ---------------------------------------------------------------------------
# Graph-free temporal relational preprocessing
# ---------------------------------------------------------------------------
...
def percentile_confidence(
    ...
    if not np.isfinite(lo) or not np.isfinite(hi):
        raise ValueError("Non-finite percentile threshold in relational branching profile.")
    if hi <= lo + 1e-12:
...
def build_metric_space(
    ...
    if config.dcs_metric_source == "observation":
        print(
            f"Relational metric space: normalized observations, dim={observations.shape[1]} "
...
class RelationalBranchStatisticsNetwork(nn.Module):
    """Predicts action diversity, continuation diversity, and action-outcome coupling.

    All three outputs are constrained to [0, 1]. The supervision is not point-wise
    action regression: targets are built from actual relational comparisons against
    a stochastic reference queue in temporal representation space.
    """
    ...
    def __call__(self, z: jnp.ndarray) -> jnp.ndarray:
        ...
        for _ in range(self.n_hidden):
            ...
            x = nn.relu(x)
        logits = nn.Dense(3, name="branch_statistics")(x)
...
def _normalized_displacements(
    ...
    return np.where(norms > 1e-8, disp / np.maximum(norms, 1e-8), 0.0).astype(np.float32)
...
def build_branch_signature(
    ...
    temporal_sig = build_temporal_signature(config, observations_shape)
    return {
        "cache_version": 1,
        "env": config.env,
        ...
        "hidden_dim": int(config.branch_hidden_dim),
        "layers": int(config.branch_layers),
        "steps": int(config.branch_steps),
        "anchor_batch_size": int(config.branch_anchor_batch_size),
        "reference_batch_size": int(config.branch_reference_batch_size),
        "lr": float(config.branch_lr),
        "weight_decay": float(config.branch_weight_decay),
        "relation_temperature": float(config.branch_relation_temperature),
        "action_coef": float(config.branch_action_coef),
        "displacement_coef": float(config.branch_displacement_coef),
        "coupling_coef": float(config.branch_coupling_coef),
        "action_scale_multiplier": float(config.branch_action_scale_multiplier),
        "branch_seed": int(config.branch_seed),
    }


def get_branch_profile_cache_path(
    config: TrainConfig,
    observations_shape: Tuple[int, ...],
    actions_shape: Tuple[int, ...],
) -> Optional[Path]:
    if config.dcs_profile_path is None:
        return None
    sig = build_branch_signature(config, observations_shape, actions_shape)
    payload = json.dumps(sig, sort_keys=True, separators=(",", ":")).encode("utf-8")
    sig_hash = hashlib.sha1(payload).hexdigest()[:12]
    root = Path(config.dcs_profile_path)
    env_name = _safe_path_name(config.env)
    filename = f"relational_branch_profile_{sig_hash}.npz"
    if config.dcs_cache_by_seed:
        return root / env_name / f"seed_{config.seed}" / filename
    return root / env_name / filename


def _global_action_spread_scale(actions: np.ndarray, multiplier: float) -> float:
    actions = np.asarray(actions, dtype=np.float32)
    centered = actions - np.mean(actions, axis=0, keepdims=True)
    scale = float(np.sqrt(np.mean(np.sum(centered ** 2, axis=-1))))
    return max(scale * float(multiplier), 1e-6)


def train_relational_branch_predictor(
    config: TrainConfig,
    ...
    next_observations: np.ndarray,
    device: Any,
) -> Tuple[RelationalBranchStatisticsNetwork, Any, Dict[str, float]]:
    """Distill stochastic soft-neighborhood relational statistics into h_psi(z).

    The teacher uses actual actions and actual transition directions from a freshly
    sampled reference queue at every step. Thus it preserves relational information
    while avoiding a persistent all-sample kNN graph.
    """

    embeddings = np.asarray(embeddings, dtype=np.float32)
    actions = np.asarray(actions, dtype=np.float32)
    unit_disp = _normalized_displacements(observations, next_observations)

    n, embed_dim = embeddings.shape
    if n < 2:
        raise ValueError("RBG-IQL needs at least two transitions for relational statistics.")

    network = RelationalBranchStatisticsNetwork(
        hidden_dim=config.branch_hidden_dim,
        n_hidden=config.branch_layers,
    )
    key = jax.random.PRNGKey(config.branch_seed)
    ...
    tx = optax.adamw(
        learning_rate=config.branch_lr,
        weight_decay=config.branch_weight_decay,
    )
    opt_state = tx.init(params)
    params = tree_to_device(params, device)
    opt_state = tree_to_device(opt_state, device)

    relation_temp = float(config.branch_relation_temperature)
    action_coef = float(config.branch_action_coef)
    disp_coef = float(config.branch_displacement_coef)
    coupling_coef = float(config.branch_coupling_coef)
    action_scale = _global_action_spread_scale(
        actions, config.branch_action_scale_multiplier
    )
    eps = 1e-8
    ...
    def train_step(
        ...
        # Normalize only for relational similarity. The prediction head itself sees
        # the original metric representation z_anchor.
        za = z_anchor / (jnp.linalg.norm(z_anchor, axis=-1, keepdims=True) + eps)
        zr = z_ref / (jnp.linalg.norm(z_ref, axis=-1, keepdims=True) + eps)
        logits = (za @ zr.T) / relation_temp

        # Exclude an exact sample match when the same dataset index appears in both
        # anchor and stochastic reference batches.
        self_mask = anchor_idx[:, None] == ref_idx[None, :]
        logits = jnp.where(self_mask, -1e9, logits)
        alpha = jax.nn.softmax(logits, axis=-1)

        # Weighted first/second moments without materializing B x R x D tensors.
        a_mean = alpha @ a_ref
        u_mean = alpha @ u_ref
        a_second = alpha @ jnp.sum(a_ref ** 2, axis=-1)
        u_second = alpha @ jnp.sum(u_ref ** 2, axis=-1)
        action_var = jnp.maximum(a_second - jnp.sum(a_mean ** 2, axis=-1), 0.0)
        outcome_var = jnp.maximum(u_second - jnp.sum(u_mean ** 2, axis=-1), 0.0)

        action_div = jnp.clip(jnp.sqrt(action_var + eps) / action_scale, 0.0, 1.0)
        # For unit displacement directions, ||E[u]|| is 1 for aligned outcomes and
        # approaches 0 when local continuation directions are diverse.
        disp_div = jnp.clip(1.0 - jnp.linalg.norm(u_mean, axis=-1), 0.0, 1.0)

        # C_au = E[a u^T] - E[a]E[u]^T. Its normalized Frobenius norm measures
        # whether different actions are systematically associated with different
        # local continuation directions, rather than diversity arising independently.
        au_second = jnp.einsum("br,rd,re->bde", alpha, a_ref, u_ref)
        cross_cov = au_second - a_mean[:, :, None] * u_mean[:, None, :]
        cross_norm_sq = jnp.sum(cross_cov ** 2, axis=(1, 2))
        coupling = jnp.sqrt(
            cross_norm_sq / (action_var * outcome_var + eps)
        )
        coupling = jnp.clip(coupling, 0.0, 1.0)

        teacher = jax.lax.stop_gradient(
            jnp.stack([action_div, disp_div, coupling], axis=-1)
        )

        def loss_fn(p):
            pred = network.apply({"params": p}, z_anchor)
            per_component = jnp.mean((pred - teacher) ** 2, axis=0)
            total = (
                action_coef * per_component[0]
                + disp_coef * per_component[1]
                + coupling_coef * per_component[2]
            )
            return total, (pred, teacher, per_component, alpha)
    ...
    rng = np.random.default_rng(config.branch_seed)
    anchor_bs = min(int(config.branch_anchor_batch_size), n)
    ref_bs = min(max(int(config.branch_reference_batch_size), 2), n)

    print(
        f"Training relational branch predictor: N={n}, embed_dim={embed_dim}, "
        f"steps={config.branch_steps}, anchors={anchor_bs}, references={ref_bs}, "
        f"T_rel={relation_temp}, action_scale={action_scale:.6f}"
    )

    running = {
        "loss": 0.0,
        "teacher_action": 0.0,
        "teacher_displacement": 0.0,
        "teacher_coupling": 0.0,
        "soft_effective_neighbors": 0.0,
    }

    for step in range(int(config.branch_steps)):
        anchor_idx_np = rng.integers(0, n, size=anchor_bs, dtype=np.int32)
        ref_idx_np = rng.choice(n, size=ref_bs, replace=(ref_bs > n)).astype(np.int32)

        z_anchor = tree_to_device(jnp.asarray(embeddings[anchor_idx_np]), device)
        z_ref = tree_to_device(jnp.asarray(embeddings[ref_idx_np]), device)
        a_ref = tree_to_device(jnp.asarray(actions[ref_idx_np]), device)
        u_ref = tree_to_device(jnp.asarray(unit_disp[ref_idx_np]), device)
        anchor_idx_j = tree_to_device(jnp.asarray(anchor_idx_np), device)
        ref_idx_j = tree_to_device(jnp.asarray(ref_idx_np), device)
        ...
        pred, teacher, per_component, alpha = aux

        if step == 0 or (step + 1) % 5_000 == 0 or step + 1 == config.branch_steps:
            loss_f = float(jax.device_get(loss))
            teacher_np = np.asarray(jax.device_get(teacher), dtype=np.float32)
            comp_np = np.asarray(jax.device_get(per_component), dtype=np.float32)
            alpha_np = np.asarray(jax.device_get(alpha), dtype=np.float32)
            soft_ess = 1.0 / np.maximum(np.sum(alpha_np ** 2, axis=1), 1e-8)
            print(
                f"Branch step {step + 1}/{config.branch_steps} | "
                f"loss={loss_f:.6f} | mse(A,P,R)="
                f"({comp_np[0]:.6f}, {comp_np[1]:.6f}, {comp_np[2]:.6f}) | "
                f"teacher_mean(A,P,R)=({teacher_np[:,0].mean():.4f}, "
                f"{teacher_np[:,1].mean():.4f}, {teacher_np[:,2].mean():.4f}) | "
                f"soft_ESS={soft_ess.mean():.2f}"
            )
            running = {
                "loss": loss_f,
                "teacher_action": float(teacher_np[:, 0].mean()),
                "teacher_displacement": float(teacher_np[:, 1].mean()),
                "teacher_coupling": float(teacher_np[:, 2].mean()),
                "soft_effective_neighbors": float(soft_ess.mean()),
            }

    return network, params, running


def predict_relational_branch_statistics(
    config: TrainConfig,
    network: RelationalBranchStatisticsNetwork,
    params: Any,
    embeddings: np.ndarray,
    ...
    chunk = int(config.branch_inference_chunk_size)
    ...
    for start in range(0, n, chunk):
        ...
        pred = np.asarray(jax.device_get(apply_fn(z)), dtype=np.float32)
        parts.append(pred)

    all_pred = np.concatenate(parts, axis=0)
    return all_pred[:, 0], all_pred[:, 1], all_pred[:, 2]


def build_relational_branch_gate(
    config: TrainConfig,
    action_diversity: np.ndarray,
    disp_diversity: np.ndarray,
    coupling: np.ndarray,
) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    action_conf = percentile_confidence(
        action_diversity, config.dcs_percentile_low, config.dcs_percentile_high
    )
    disp_conf = percentile_confidence(
        disp_diversity, config.dcs_percentile_low, config.dcs_percentile_high
    )
    coupling_conf = percentile_confidence(
        coupling, config.dcs_percentile_low, config.dcs_percentile_high
    )
    ...
    if config.dcs_gate_source == "action":
        ...
        signal = product
    elif config.dcs_gate_source == "coupling":
        signal = coupling_conf
    elif config.dcs_gate_source == "coupled_product":
        signal = (
            product
            * np.power(
                np.clip(coupling_conf, 0.0, 1.0),
                float(config.branch_coupling_power),
            )
        ).astype(np.float32)
    else:
        raise ValueError(
            f"Unsupported dcs_gate_source for {ALGORITHM_NAME}: {config.dcs_gate_source}"
        )
    ...
    profile = {
        "action_diversity": np.asarray(action_diversity, dtype=np.float32),
        "disp_diversity": np.asarray(disp_diversity, dtype=np.float32),
        "coupling": np.asarray(coupling, dtype=np.float32),
        "action_conf": action_conf,
        "disp_conf": disp_conf,
        "coupling_conf": coupling_conf,
        "product_diversity": product,
        "signal": signal,
        "gate": gate,
    }
    return gate, profile
...
def load_or_compute_relational_branch_gate(
    config: TrainConfig,
    dataset: Dict[str, np.ndarray],
    device: Any,
) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    next_observations = np.asarray(dataset["next_observations"], dtype=np.float32)
    cache_path = get_branch_profile_cache_path(
    ...
    if (
        ...
        and not config.branch_force_recompute
        and not config.dcs_force_recompute
    ):
        try:
            with np.load(cache_path, allow_pickle=False) as data:
                action_diversity = np.asarray(data["action_diversity"], dtype=np.float32)
                disp_diversity = np.asarray(data["disp_diversity"], dtype=np.float32)
                coupling = np.asarray(data["coupling"], dtype=np.float32)
            if (
                action_diversity.shape[0] == observations.shape[0]
                and disp_diversity.shape[0] == observations.shape[0]
                and coupling.shape[0] == observations.shape[0]
            ):
                gate, profile = build_relational_branch_gate(
                    config, action_diversity, disp_diversity, coupling
                )
                print(f"Loaded cached relational branching profile from: {cache_path}")
                return gate, profile
        except Exception as exc:
            print(f"Ignoring invalid relational branching cache {cache_path}: {exc}")

    embeddings = build_metric_space(config, dataset, device=device)
    network, params, train_diag = train_relational_branch_predictor(
        config=config,
    ...
    action_diversity, disp_diversity, coupling = predict_relational_branch_statistics(
    ...
    if cache_path is not None:
        ...
        np.savez_compressed(
            cache_path,
            action_diversity=action_diversity.astype(np.float32),
            disp_diversity=disp_diversity.astype(np.float32),
            coupling=coupling.astype(np.float32),
        )
        print(f"Saved relational branching profile to: {cache_path}")

    print(
        f"{ALGORITHM_NAME} relational profile | gate_source={config.dcs_gate_source} | "
        f"pred_mean(A,P,R)=({float(np.mean(action_diversity)):.4f}, "
        f"{float(np.mean(disp_diversity)):.4f}, {float(np.mean(coupling)):.4f}) | "
        f"signal mean={float(np.mean(profile['signal'])):.4f} | "
        f"gate mean={float(np.mean(gate)):.4f} | "
        f"active-row frac={float(np.mean(gate > 1e-6)):.4f} | "
        f"last teacher soft_ESS={train_diag.get('soft_effective_neighbors', float('nan')):.2f}"
    )
    return gate, profile
...
class ReplayBuffer:
    """Offline replay buffer with one precomputed relational branching gate G_i per transition."""
    ...
    def load_d4rl_dataset(
        ...
        print(
            f"Dataset size: {n_transitions} | graph-free relational branching gate | "
...
class RelationalBranchingGatedIQLJAX:
    """Relational Branching-Gated IQL in JAX/Flax.

    Q-function training and actor extraction mirror standard IQL. The only
    departure is value learning: tau(s) is controlled by a precomputed, graph-free
    ...
    def _build_train_step(self):
        ...
        def train_step(state: IQLState, batch: TensorBatch):
            ...
            # RBG-IQL does NOT compute kernel ESS. The ReplayBuffer stores the
            # state-level relational branching gate G_i in the legacy "junction" field.
            ...
            log_dict = {
                ...
                # RBG-IQL diagnostics: no kNN/ESS/neighbor tensors exist.
...
def train(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="rbg_iql_jax_training_progress",
    ...
    # ----- RBG-IQL preprocessing: temporal embedding + relational distillation -----
    gate = None
    if config.use_dcs:
        gate, _branch_profile = load_or_compute_relational_branch_gate(
            config, dataset, device=jax_device
        )
    else:
        print(
            "use_dcs=False: relational branching gate disabled; tau(s)=scv_tau_min. "
    ...
    print(
        ...
        f"use_relational_branch_gate={config.use_dcs} | support_power={config.scv_support_power} | "
        f"metric={config.dcs_metric_source} | gate_source={config.dcs_gate_source}"
    )
    if config.use_dcs:
        print(
            "support confidence: c_i = G_i^p from distilled relational branching statistics; "
            "no full-dataset kNN graph, no density, no kernel ESS, no neighbor pooling"
        )
        if config.dcs_metric_source == "temporal":
            print(
                f"*** TEMPORAL RELATIONAL BRANCHING METRIC: phi(s) dim={config.dcs_temporal_dim}, "
                f"horizon={config.dcs_temporal_horizon}, steps={config.dcs_temporal_steps}, "
                f"representation_seed={config.dcs_temporal_seed}; graph-free distillation. ***"
            )
        else:
            print("*** OBSERVATION RELATIONAL BRANCHING METRIC: normalized observation input; graph-free. ***")
    print("---------------------------------------")

    trainer = RelationalBranchingGatedIQLJAX(
