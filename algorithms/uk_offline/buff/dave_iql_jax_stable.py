...
# This revision adds stability fixes from log analysis: normalized actor weights,
# delay warmup, scheduled value filtering, bootstrap critic decoupling,
# gradient clipping, and optional target-value clipping.
#
# Main idea:
...
class TrainConfig:
    ...
    # Stability fixes from DAVE-IQL log analysis.
    # delay_warmup_steps: while critics are untrained, keep delayed nets synced every step.
    delay_warmup_steps: int = 10000

    # Scheduled V filtering. If init values are None, the final values above are used from step 1.
    # Otherwise linearly interpolate init -> final over v_filter_schedule_steps.
    v_filter_temperature_init: Optional[float] = None
    v_filter_floor_init: Optional[float] = None
    v_filter_schedule_steps: int = 0

    # Actor AWBC stabilization. During warmup the actor is trained by plain BC.
    # After warmup, normalized clipped advantage weights are used.
    actor_awbc_warmup_steps: int = 50000
    actor_adv_temperature: float = 3.0
    actor_adv_clip: float = 2.0
    actor_weight_floor: float = 0.0
    actor_weight_max: float = 20.0
    actor_weight_baseline: str = "mean"  # one of: mean, median, zero
    actor_weight_normalize: bool = True

    # Bootstrap-style critic decoupling: Q_select and Q_eval see different random subsets.
    bootstrap_decouple: bool = True
    bootstrap_select_prob: float = 0.5

    # Optimizer / target stabilization.
    grad_clip_norm: float = 10.0
    target_value_clip: bool = False
    target_value_min: float = -100.0
    target_value_max: float = 0.0
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.delay_warmup_steps >= 0
    ...
    if config.v_filter_temperature_init is not None:
        assert config.v_filter_temperature_init > 0.0
    if config.v_filter_floor_init is not None:
        assert config.v_filter_floor_init >= 0.0 and config.v_filter_floor_init <= 1.0
    assert config.v_filter_schedule_steps >= 0
    assert config.actor_awbc_warmup_steps >= 0
    assert config.actor_adv_temperature > 0.0
    assert config.actor_adv_clip > 0.0
    assert config.actor_weight_floor >= 0.0 and config.actor_weight_floor <= 1.0
    assert config.actor_weight_max > 0.0
    assert config.actor_weight_baseline in ("mean", "median", "zero")
    assert config.bootstrap_select_prob > 0.0 and config.bootstrap_select_prob < 1.0
    assert config.grad_clip_norm >= 0.0
    assert config.target_value_min <= config.target_value_max
...
class DAVEIQLJAX:
    ...
    def __init__(
        ...
        actor_weight_floor: float = 0.0,
        actor_weight_max: float = 20.0,
        actor_weight_baseline: str = "mean",
        actor_weight_normalize: bool = True,
        bootstrap_decouple: bool = True,
        bootstrap_select_prob: float = 0.5,
        grad_clip_norm: float = 10.0,
        ...
        self.v_filter_temperature_init = float(v_filter_temperature if v_filter_temperature_init is None else v_filter_temperature_init)
        self.v_filter_floor_init = float(v_filter_floor if v_filter_floor_init is None else v_filter_floor_init)
        self.v_filter_schedule_steps = int(v_filter_schedule_steps)
        self.actor_awbc_warmup_steps = int(actor_awbc_warmup_steps)
        self.actor_adv_temperature = float(actor_adv_temperature)
        self.actor_adv_clip = float(actor_adv_clip)
        self.actor_weight_floor = float(actor_weight_floor)
        self.actor_weight_max = float(actor_weight_max)
        self.actor_weight_baseline = actor_weight_baseline
        self.actor_weight_normalize = bool(actor_weight_normalize)
        self.bootstrap_decouple = bool(bootstrap_decouple)
        self.bootstrap_select_prob = float(bootstrap_select_prob)
        self.grad_clip_norm = float(grad_clip_norm)
        self.target_value_clip = bool(target_value_clip)
        self.target_value_min = float(target_value_min)
        self.target_value_max = float(target_value_max)
        ...
        def make_tx(base_tx):
            if self.grad_clip_norm > 0.0:
                return optax.chain(optax.clip_by_global_norm(self.grad_clip_norm), base_tx)
            return base_tx

        self.q_select_tx = make_tx(optax.adam(qf_lr))
        self.q_eval_tx = make_tx(optax.adam(qf_lr))
        self.v_tx = make_tx(optax.adam(vf_lr))
        actor_lr_schedule = optax.cosine_decay_schedule(
            init_value=actor_lr,
            decay_steps=max(int(max_steps), 1),
            alpha=0.0,
        )
        self.actor_tx = make_tx(optax.adam(actor_lr_schedule))
    ...
    def _build_train_step(self):
        ...
        delay_warmup_steps = self.delay_warmup_steps
        v_filter_temperature_final = self.v_filter_temperature
        v_filter_temperature_init = self.v_filter_temperature_init
        v_filter_clip = self.v_filter_clip
        v_filter_floor_final = self.v_filter_floor
        v_filter_floor_init = self.v_filter_floor_init
        v_filter_schedule_steps = self.v_filter_schedule_steps
        v_filter_baseline = self.v_filter_baseline
        actor_awbc_warmup_steps = self.actor_awbc_warmup_steps
        actor_adv_temperature = self.actor_adv_temperature
        actor_adv_clip = self.actor_adv_clip
        actor_weight_floor = self.actor_weight_floor
        actor_weight_max = self.actor_weight_max
        actor_weight_baseline = self.actor_weight_baseline
        actor_weight_normalize = self.actor_weight_normalize
        bootstrap_decouple = self.bootstrap_decouple
        bootstrap_select_prob = self.bootstrap_select_prob
        target_value_clip = self.target_value_clip
        ...
        def baseline_for(values: jnp.ndarray, mode: str) -> jnp.ndarray:
            if mode == "mean":
        ...
        def train_step(state: IQLState, batch: TensorBatch):
            ...
            batch_size_f = jnp.asarray(observations.shape[0], dtype=jnp.float32)

            # One RNG stream drives bootstrap masks and dropout, so the masks are reproducible
            # through the training state and remain JIT-compatible.
            actor_key, dropout_key, bootstrap_key = jax.random.split(state.actor_key, 3)
            ...
            adv_baseline = baseline_for(adv_select_delay, v_filter_baseline)

            # Scheduled soft filtering. Early training can stay close to uniform mean,
            # then gradually become more selective as critics become more reliable.
            if v_filter_schedule_steps > 0:
                progress = jnp.clip(
                    total_it.astype(jnp.float32) / jnp.asarray(v_filter_schedule_steps, dtype=jnp.float32),
                    0.0,
                    1.0,
                )
                current_v_filter_temperature = (
                    v_filter_temperature_init
                    + progress * (v_filter_temperature_final - v_filter_temperature_init)
                )
                current_v_filter_floor = v_filter_floor_init + progress * (v_filter_floor_final - v_filter_floor_init)
            else:
                current_v_filter_temperature = jnp.asarray(v_filter_temperature_final, dtype=jnp.float32)
                current_v_filter_floor = jnp.asarray(v_filter_floor_final, dtype=jnp.float32)
            ...
            if target_value_clip:
                target_q_for_backup = jnp.clip(target_q_for_backup, target_value_min, target_value_max)
                q_eval_target_for_v = jnp.clip(q_eval_target_for_v, target_value_min, target_value_max)
            target_q_for_backup = jax.lax.stop_gradient(target_q_for_backup)

            # Bootstrap-style critic decoupling. Q_select and Q_eval are not used as
            # min/max twins; they are trained on different stochastic subsets to reduce
            # selection/evaluation error correlation.
            if bootstrap_decouple:
                select_mask = jax.random.bernoulli(
                    bootstrap_key,
                    p=bootstrap_select_prob,
                    shape=rewards.shape,
                ).astype(jnp.float32)
                eval_mask = 1.0 - select_mask
            else:
                select_mask = jnp.ones_like(rewards)
                eval_mask = jnp.ones_like(rewards)
            select_mask_sum = jnp.sum(select_mask)
            eval_mask_sum = jnp.sum(eval_mask)
            q_select_mask_frac = jnp.mean(select_mask)
            q_eval_mask_frac = jnp.mean(eval_mask)

            def q_select_loss_fn(q_select_params):
                q_select = q_apply({"params": q_select_params}, observations, actions)
                td = (q_select - target_q_for_backup) ** 2
                q_select_loss = jnp.sum(select_mask * td) / (select_mask_sum + eps)
            ...
            def q_eval_loss_fn(q_eval_params):
                ...
                td = (q_eval - target_q_for_backup) ** 2
                q_eval_loss = jnp.sum(eval_mask * td) / (eval_mask_sum + eps)
            ...
            # Actor update: normalized AWR/IQL-style weighted BC. During warmup,
            # plain BC is used to avoid early stale-delayed-advantage damage.
            actor_adv_baseline = baseline_for(adv_select_delay, actor_weight_baseline)
            actor_weight_logits = (
                beta
                * (jax.lax.stop_gradient(adv_select_delay) - jax.lax.stop_gradient(actor_adv_baseline))
                / actor_adv_temperature
            )
            actor_weight_logits = jnp.clip(actor_weight_logits, -actor_adv_clip, actor_adv_clip)
            raw_actor_weights = jnp.exp(actor_weight_logits)
            if actor_weight_normalize:
                actor_weights = raw_actor_weights / (jnp.mean(raw_actor_weights) + eps)
            else:
                actor_weights = raw_actor_weights
            actor_weights = actor_weight_floor + (1.0 - actor_weight_floor) * actor_weights
            actor_weights = jnp.clip(actor_weights, 0.0, actor_weight_max)
            actor_awbc_active = total_it > jnp.asarray(actor_awbc_warmup_steps, dtype=jnp.int32)
            actor_weights = jax.lax.cond(
                actor_awbc_active,
                lambda _: actor_weights,
                lambda _: jnp.ones_like(actor_weights),
                operand=jnp.asarray(0, dtype=jnp.int32),
            )
            actor_weights = jax.lax.stop_gradient(actor_weights)
            actor_weight_sum = jnp.sum(actor_weights)
            actor_weight_ess = (actor_weight_sum ** 2) / (jnp.sum(actor_weights ** 2) + eps)
            actor_weight_ess_ratio = actor_weight_ess / batch_size_f
            ...
            def actor_loss_fn(actor_params):
                ...
                actor_loss = jnp.mean(actor_weights * bc_losses)
            ...
            delay_warmup_active = total_it <= jnp.asarray(delay_warmup_steps, dtype=jnp.int32)
            periodic_sync = (total_it % jnp.asarray(delayed_update_freq, dtype=jnp.int32)) == 0
            should_sync = jnp.logical_or(delay_warmup_active, periodic_sync)
            ...
            log_dict = {
                ...
                "q_select_mask_frac": q_select_mask_frac,
                "q_eval_mask_frac": q_eval_mask_frac,
                "target_q_mean": jnp.mean(target_q_for_backup),
                "target_q_min": jnp.min(target_q_for_backup),
                "target_q_max": jnp.max(target_q_for_backup),
                "target_value_clipped": jnp.asarray(1.0 if target_value_clip else 0.0, dtype=jnp.float32),
                ...
                "v_filter_temperature_current": current_v_filter_temperature,
                "v_filter_floor_current": current_v_filter_floor,
                ...
                "delay_warmup_active": delay_warmup_active.astype(jnp.float32),
                "actor_awbc_active": actor_awbc_active.astype(jnp.float32),
                "actor_weight_mean": jnp.mean(actor_weights),
                "actor_weight_std": jnp.std(actor_weights),
                "actor_weight_min": jnp.min(actor_weights),
                "actor_weight_max": jnp.max(actor_weights),
                "actor_weight_ess_ratio": actor_weight_ess_ratio,
                # Backward-compatible log keys. These are normalized actor weights now, not raw exp(beta*A).
                "exp_adv_mean": jnp.mean(actor_weights),
                "exp_adv_max": jnp.max(actor_weights),
    ...
    def _build_actor_refit_step(self):
        ...
        def actor_refit_step(actor_state: ActorState, iql_state: IQLState, batch: TensorBatch):
            ...
            adv_baseline = baseline_for(adv, actor_weight_baseline)
            weight_logits = beta * (jax.lax.stop_gradient(adv) - jax.lax.stop_gradient(adv_baseline)) / actor_adv_temperature
            weight_logits = jnp.clip(weight_logits, -actor_adv_clip, actor_adv_clip)
            raw_weights = jnp.exp(weight_logits)
            if actor_weight_normalize:
                actor_weights = raw_weights / (jnp.mean(raw_weights) + eps)
            else:
                actor_weights = raw_weights
            actor_weights = actor_weight_floor + (1.0 - actor_weight_floor) * actor_weights
            actor_weights = jnp.clip(actor_weights, 0.0, actor_weight_max)
            actor_weights = jax.lax.stop_gradient(actor_weights)
            actor_weight_sum = jnp.sum(actor_weights)
...
def train(config: TrainConfig):
    ...
    trainer = DAVEIQLJAX(
        ...
        delay_warmup_steps=config.delay_warmup_steps,
        v_filter_temperature_init=config.v_filter_temperature_init,
        v_filter_floor_init=config.v_filter_floor_init,
        v_filter_schedule_steps=config.v_filter_schedule_steps,
        actor_awbc_warmup_steps=config.actor_awbc_warmup_steps,
        actor_adv_temperature=config.actor_adv_temperature,
        actor_adv_clip=config.actor_adv_clip,
        actor_weight_floor=config.actor_weight_floor,
        actor_weight_max=config.actor_weight_max,
        actor_weight_baseline=config.actor_weight_baseline,
        actor_weight_normalize=config.actor_weight_normalize,
        bootstrap_decouple=config.bootstrap_decouple,
        bootstrap_select_prob=config.bootstrap_select_prob,
        grad_clip_norm=config.grad_clip_norm,
        target_value_clip=config.target_value_clip,
        target_value_min=config.target_value_min,
        target_value_max=config.target_value_max,
        seed=seed,
        device=jax_device,
    )
