    ...
    def _build_common_fns(self):
        ...
        def compute_actor_loss(actor_params, observations, actions, q_params, q_delayed_params, v_delayed_params, rng):
            ...
            if actor_update_method in IQL_ACTOR_METHODS:
                # Match iql_jax exactly for IQL-style actor extraction:
                #   exp_adv = min(exp(beta * stop_gradient(adv)), EXP_ADV_MAX)
                #   actor_loss = mean(stop_gradient(exp_adv) * BC)
                policy_weight = jnp.minimum(
                    jnp.exp(beta * jax.lax.stop_gradient(data_adv)),
                    jnp.asarray(EXP_ADV_MAX, dtype=jnp.float32),
                )
