...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.n_episodes > 0, "n_episodes must be > 0 for evaluation; use eval_freq > max_timesteps to disable eval"
...
class CDAFJAX:
    ...
    def eval_actor(self, env: gym.Env, actor_params: Any, n_episodes: int, seed: int) -> np.ndarray:
        if n_episodes <= 0:
            raise ValueError(
                "n_episodes must be > 0 for evaluation. "
                "Current value produced an empty eval_scores array."
            )
        ...
        for fit_step in range(1, steps + 1):
            ...
            if should_eval:
                ...
                if eval_scores.size == 0:
                    raise RuntimeError(
                        "eval_scores is empty during actor refit. "
                        "Check that eval_episodes > 0."
                    )
...
def train(config: TrainConfig):
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            if eval_scores.size == 0:
                raise RuntimeError(
                    "eval_scores is empty. Check that config.n_episodes > 0 "
                    "and that eval_actor appends one return per episode."
                )
