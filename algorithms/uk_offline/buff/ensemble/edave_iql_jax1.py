...
class TrainConfig:
    ...
    # Supported modes follow the attached CDAF-style code:
    ...
    # Backward-compatible aliases for previous EDAVE actor weighting fields.
    # If old YAML files set these, they are mapped to the actor_update_method parameters.
    beta: float = 3.0
    actor_weight_temperature: float = 3.0
    actor_weight_clip: float = 3.0
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.actor_weight_temperature > 0.0
    assert config.actor_weight_clip > 0.0
...
def apply_env_hyperparams(config: TrainConfig) -> TrainConfig:
    ...
    aliases = {
        "n_timesteps": "max_timesteps",
        # Backward compatibility for old actor-refit YAML files.
        "actor_fit_method": "actor_update_method",
        "eval_actor_steps": "max_timesteps",
        "eval_actor_batch_size": "batch_size",
        "eval_actor_eval_freq": "eval_freq",
...
class EDAVEIQLJAX:
    ...
    Main algorithm:
      ...
      - Actor losses support td3_bc, weighted_bc, td3_weighted_bc, iql, and iql_awbc.
    ...
    def __init__(
        ...
        alpha: float = 2.5,
        bc_coef: float = 1.0,
        # Backward-compatible fields from previous EDAVE actor AWBC.
        beta: float = 3.0,
        actor_weight_temperature: float = 3.0,
        actor_weight_clip: float = 3.0,
        actor_weight_floor: float = 0.0,
        ...
        # If user leaves policy_weight_exponent at default but sets old beta, beta still works for IQL modes.
        self.policy_weight_exponent = float(policy_weight_exponent if policy_weight_exponent is not None else beta)
        self.policy_weight_clip = float(policy_weight_clip)
        self.alpha = float(alpha)
        self.bc_coef = float(bc_coef)
        self.beta = float(beta)
        self.actor_weight_temperature = float(actor_weight_temperature)
    ...
    def _build_common_fns(self):
        ...
        actor_weight_temperature = self.actor_weight_temperature
        ...
        def stack_q(params_tuple, observations, actions):
            return jnp.stack(
                [q_apply({"params": params_tuple[i]}, observations, actions) for i in range(ensemble_size)],
                axis=0,
            )

        def stack_v(params_tuple, observations):
            return jnp.stack(
                [v_apply({"params": params_tuple[i]}, observations) for i in range(ensemble_size)],
                axis=0,
            )
        ...
        def compute_actor_loss(actor_params, observations, actions, q_params, q_delayed_params, v_delayed_params, rng):
            ...
            if actor_update_method in IQL_ACTOR_METHODS:
                ...
                logits = jnp.clip(
                    policy_weight_exponent * data_adv / actor_weight_temperature,
                    -actor_weight_clip,
                    actor_weight_clip,
                )
                policy_weight = jnp.exp(logits)
                ...
                policy_weight = actor_weight_floor + (1.0 - actor_weight_floor) * policy_weight
                policy_weight = jnp.minimum(policy_weight, jnp.minimum(policy_weight_clip, actor_weight_max))
                weighted_bc_loss = jnp.mean(jax.lax.stop_gradient(policy_weight) * bc_per_sample)
                weight_mean = jnp.mean(policy_weight)
                weight_max = jnp.max(policy_weight)
            else:
    ...
    def _build_train_step(self):
        ...
        def train_step(state: EDAVEState, actor_state: ActorState, batch: TensorBatch):
            ...
            v_weight_logits = jnp.clip(
                jax.lax.stop_gradient(adv_filter_all) / v_filter_temperature,
                -v_filter_clip,
                v_filter_clip,
            )
    ...
    def make_loaded_actor_state(self) -> ActorState:
        return tree_to_device(
            ActorState(
                params=copy.deepcopy(self.actor_state.params),
                opt_state=copy.deepcopy(self.actor_state.opt_state),
                key=copy.deepcopy(self.actor_state.key),
            ),
            self.device,
        )
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]):
        state_key = "edave_state" if "edave_state" in state_dict else "iql_state"
...
DAVEIQLJAX = EDAVEIQLJAX
SDAFIQLJAX = EDAVEIQLJAX
IQLJAX = EDAVEIQLJAX
...
def train(config: TrainConfig):
    ...
    trainer = EDAVEIQLJAX(
        ...
        beta=config.beta,
        actor_weight_temperature=config.actor_weight_temperature,
        actor_weight_clip=config.actor_weight_clip,
