# JAX/Flax Sparse Q-Learning (SQL) implementation with CDAF_JAX-style experiment plumbing.
# SQL losses follow the ICLR 2023 IVR paper equations (12), (13), and (14);
# checkpointing, hyperparameter merging, logging, and path handling follow the supplied IQL framework.
#
# Current plumbing:
...
ALGORITHM_NAME = "SQL"
ALGORITHM_FULL_NAME = "Sparse Q-Learning"
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert config.alpha_v > 0.0
    assert config.alpha_pi > 0.0
...
def apply_env_hyperparams(config: TrainConfig) -> TrainConfig:
    ...
    if skipped_unknown:
        print(f"Ignored unknown hyperparameter keys for SQL: {', '.join(skipped_unknown)}")
    return config
...
class SQLJAX:
    """Sparse Q-Learning in JAX/Flax.

    The value, Q, and actor update equations implement SQL Eq. (12), Eq. (13),
    and Eq. (14) on top of the supplied JAX/Flax IQL experiment framework.
    """
    ...
    def _build_train_step(self):
        ...
        @jax.jit
        def train_step(state: SQLState, batch: TensorBatch):
            ...
            # SQL Eq. (12): V update from in-dataset actions only, using clipped double target Q.
            ...
            def v_loss_fn(v_params):
                ...
                sparse_term = sql_sparse_term(value_adv, alpha_v)
                sparse_mask = (sparse_term > 0.0).astype(jnp.float32)
                value_loss = jnp.mean(sparse_mask * sparse_term ** 2 + v / alpha_v)
                return value_loss, (v, value_adv, sparse_term, sparse_mask)

            (value_loss, (v_for_loss, value_adv, value_sparse_term, value_sparse_mask)), v_grads = jax.value_and_grad(
                v_loss_fn,
                has_aux=True,
            )(state.v_params)
            ...
            # SQL Eq. (13): Q Bellman backup uses the newly updated V network.
            next_v = v_apply({"params": v_params}, next_observations)
            ...
            # SQL Eq. (14): sparse weighted behavior cloning for the actor.
            actor_q1, actor_q2 = q_apply({"params": q_target_params}, observations, actions)
            actor_q = jnp.minimum(actor_q1, actor_q2)
            actor_v = v_apply({"params": v_params}, observations)
            actor_adv = actor_q - actor_v
            actor_weight = sql_sparse_weight(jax.lax.stop_gradient(actor_adv), alpha_pi)
            ...
            log_dict = {
                ...
                "v_mean": jnp.mean(v_for_loss),
                "adv_mean": jnp.mean(value_adv),
                "adv_min": jnp.min(value_adv),
                "adv_max": jnp.max(value_adv),
                "sql_value_term_mean": jnp.mean(value_sparse_term),
                "sql_value_active_frac": jnp.mean(value_sparse_mask),
                "sql_actor_weight_mean": jnp.mean(actor_weight),
                "sql_actor_weight_max": jnp.max(actor_weight),
                "sql_actor_active_frac": jnp.mean((actor_weight > 0.0).astype(jnp.float32)),
                "alpha_v": jnp.asarray(alpha_v, dtype=jnp.float32),
                "alpha_pi": jnp.asarray(alpha_pi, dtype=jnp.float32),
    ...
    def _build_actor_refit_step(self):
        ...
        @jax.jit
        def actor_refit_step(actor_state: ActorState, sql_state: SQLState, batch: TensorBatch):
            observations = batch["observations"]
            actions = batch["actions"]

            q1, q2 = q_apply({"params": sql_state.q_target_params}, observations, actions)
            target_q = jnp.minimum(q1, q2)
            v = v_apply({"params": sql_state.v_params}, observations)
            adv = target_q - v
            actor_weight = sql_sparse_weight(jax.lax.stop_gradient(adv), alpha_pi)
            ...
            log_dict = {
                ...
                "adv_min": jnp.min(adv),
                "adv_max": jnp.max(adv),
                "weight_mean": jnp.mean(actor_weight),
                "weight_max": jnp.max(actor_weight),
                "weight_active_frac": jnp.mean((actor_weight > 0.0).astype(jnp.float32)),
    ...
    def fit_actor(
        ...
        refit_log: Dict[str, Any] = {
            ...
            f"{prefix}/final_weight_mean": np.nan,
        ...
        for fit_step in range(1, steps + 1):
            ...
            refit_log[f"{prefix}/final_weight_mean"] = step_log["weight_mean"]
            ...
            if should_eval:
                ...
                print(
                    f"[{prefix}:sql_awbc] step {fit_step}/{steps}: "
                    f"loss={step_log['loss']:.4f}, bc={step_log['bc_loss']:.4f}, "
                    f"adv={step_log['adv_mean']:.4f}, weight={step_log['weight_mean']:.4f}, "
    ...
    def state_dict(self) -> Dict[str, Any]:
        return {
            "sql_state": serialization.to_state_dict(self.state),
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]):
        self.state = serialization.from_state_dict(self.state, state_dict.get("sql_state", state_dict.get("iql_state")))
...
def resolve_checkpoint_path(
    ...
    """Return (run_dir, checkpoint_path) for a saved SQL-JAX checkpoint.
...
def _train_impl(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="sql_jax_training_progress",
