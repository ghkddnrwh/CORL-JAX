...
ALGORITHM_FULL_NAME = "FQL-codepath ReBRAC with IQL_JAX-style plumbing"
...
def validate_config(config: TrainConfig) -> None:
    ...
    if config.num_critics != 2:
        print(
            "Warning: FQL ReBRAC originally uses exactly two critics. "
...
class ReBRACAgent(flax.struct.PyTreeNode):
    ...
    def update(self, batch, full_update=True):
        ...
        if full_update:
            ...
            new_network = self._target_update(new_network, "actor")
    ...
    def create(cls, seed, ex_observations, ex_actions, config):
        ...
        network_info = dict(
            ...
            target_actor=(deepcopy(actor_def), (ex_observations,)),
        )
        ...
        params["modules_target_actor"] = deepcopy(params["modules_actor"])
...
class ReBRACFQLJAX:
    def __init__(self, config: TrainConfig, ex_observations: np.ndarray, ex_actions: np.ndarray, device: Any = None):
        ...
        self.agent = ReBRACAgent.create(config.seed, jnp.asarray(ex_observations), jnp.asarray(ex_actions), self.fql_config)
    ...
    def _build_actor_refit_step(self):
        ...
        @jax.jit
        def actor_refit_step(actor_state: ActorRefitState, agent: ReBRACAgent, batch: TensorBatch):
...
def make_checkpoint_payload(trainer: ReBRACFQLJAX, config: TrainConfig, state_mean: Union[np.ndarray, float], state_std: Union[np.ndarray, float]) -> Dict[str, Any]:
    return {
...
def _train_impl(config: TrainConfig):
    ...
    if config.checkpoints_path is not None and (not actor_refit_only):
        ...
        checkpoint_manager = TrainingCheckpointManager(
            ...
            checkpoint_type="rebrac_jax_fql_implementation_training_progress",
