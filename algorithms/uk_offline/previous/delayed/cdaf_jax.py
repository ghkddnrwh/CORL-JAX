...
class TrainConfig:
    ...
    # Actor policy learning. In mode="train", the actor is updated every
    # training step together with Q and V. In mode="refit", Q/V are frozen from
    # a loaded checkpoint and only pi is optimized.
    ...
    actor_update_method: str = "td3_weighted_bc"  # one of: td3_bc, weighted_bc, td3_weighted_bc, iql, iql_awbc
    actor_lr: float = 3e-4
...
class CDAFJAX:
    ...
    cross-ensemble negative-advantage filtering. The actor is trained jointly
    with Q/V in mode="train" and actor-only in mode="refit".
    """
    def __init__(
        self,
        max_action: float,
        ...
        init_keys = jax.random.split(key, 1 + 2 * self.ensemble_size)
        key_actor = init_keys[0]
        q_keys = init_keys[1 : 1 + self.ensemble_size]
        v_keys = init_keys[1 + self.ensemble_size :]
    ...
    def _build_train_step(self):
        ...
        def cycle_shift_for_update(total_it_: jnp.ndarray) -> jnp.ndarray:
            ...
            shift = jnp.asarray(1, dtype=jnp.int32) + (delayed_round % jnp.asarray(ensemble_size - 1, dtype=jnp.int32))
        ...
        def train_step(state: CDAFState, actor_state: ActorState, batch: TensorBatch):
            ...
            # sees the freshly updated online Q/V for policy-improvement terms,
            # while dataset-action BC weights are computed from delayed Q/V.
            def actor_loss_fn(actor_params):
                ...
                if actor_update_method in IQL_ACTOR_METHODS:
                    data_q = jnp.mean(delayed_q_all, axis=0)
                    data_v = jnp.mean(delayed_v_all, axis=0)
        ...
        for fit_step in range(1, steps + 1):
            ...
            if should_eval:
                ...
                if save_dir_path is not None:
                    ...
                    print(
                        f"[{prefix}:{self.actor_update_method}] saved intermediate refit outputs to "
                        f"{save_dir_path}"
                    )
...
#                 # CDAF has a single value network, so use the same baseline as
...
# # #         @jax.jit
# # #         def actor_fit_step(actor_state: ActorState, q_params: Any, batch: TensorBatch):
# # #             observations = batch["observations"]
# # #             actions = batch["actions"]

# # #             def actor_loss_fn(actor_params):
# # #                 pi = actor_apply({"params": actor_params}, observations)
# # #                 q = q_apply({"params": q_params}, observations, pi)
# # #                 lmbda = jax.lax.stop_gradient(alpha / jnp.maximum(jnp.mean(jnp.abs(q)), 1e-6))
# # #                 bc_loss = jnp.mean((pi - actions) ** 2)
# # #                 actor_loss = -lmbda * jnp.mean(q) + bc_coef * bc_loss
# # #                 return actor_loss, (q, lmbda, bc_loss)

# # #             (actor_loss, (q, lmbda, bc_loss)), actor_grads = jax.value_and_grad(actor_loss_fn, has_aux=True)(actor_state.params)
# # #             updates, opt_state = actor_tx.update(actor_grads, actor_state.opt_state, actor_state.params)
# # #             params = optax.apply_updates(actor_state.params, updates)
# # #             new_actor_state = ActorState(params=params, opt_state=opt_state)
# # #             log_dict = {
...
# # #         for fit_step in range(1, steps + 1):
# # #             batch = replay_buffer.sample(batch_size)
# # #             actor_state, step_log = self._actor_fit_step(actor_state, q_params, batch)
# # #             step_log = {key: float(jax.device_get(value)) for key, value in step_log.items()}
