...
class TrainConfig:
    ...
    qf_lr: float = 3e-4
    actor_lr: float = 3e-4
    # IQL-style actor inverse temperature. Used only by actor_update_method in {iql, iql_awbc}.
    beta: float = 3.0
    ...
    ensemble_size: int = 2
    delayed_update_period: int = 1000
    ...
    v_filter_positive_adv_weight_one: bool = True
    ...
    # Used by weighted_bc / td3_weighted_bc. IQL-style methods use beta and EXP_ADV_MAX,
    # matching iql_jax.
...
class EDAVEIQLJAX:
    ...
    def _build_common_fns(self):
        ...
        def compute_actor_loss(actor_params, observations, actions, q_params, q_delayed_params, v_delayed_params, rng):
            ...
            if actor_update_method in IQL_ACTOR_METHODS:
                ...
                #   min(exp(beta * stop_gradient(adv)), EXP_ADV_MAX)
                # This avoids computing exp(...) before applying the upper cap.
                policy_log_weight = jnp.minimum(
                    beta * jax.lax.stop_gradient(data_adv),
                    jnp.log(jnp.asarray(EXP_ADV_MAX, dtype=jnp.float32)),
                )
                policy_weight = jnp.exp(policy_log_weight)
    ...
    def _build_train_step(self):
        ...
        def train_step(state: EDAVEState, actor_state: ActorState, batch: TensorBatch):
            ...
            should_sync = (total_it % jnp.asarray(delayed_update_period, dtype=jnp.int32)) == 0
