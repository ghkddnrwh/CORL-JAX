...
ALGORITHM_NAME = "SA-CDAF"
ALGORITHM_FULL_NAME = "Support-Aware Conservative Delayed Advantage Filtering"
...
class TrainConfig:
    ...
    hyperparams_path: Optional[str] = "hyperparams/sa_cdaf_jax.yml"
    ...
    # SA-CDAF value learning
    discount: float = 0.99
    tau: float = 0.005
    qf_lr: float = 3e-4
    vf_lr: float = 3e-4
    ensemble_size: int = 1  # number of independent Q/V pairs for ensemble double Q-learning
    ...
    # State-support gate rho(s). rho=0 treats the state as sparse and uses
    # all dataset actions for V(s); rho=1 treats local action comparison as
    # reliable and uses delayed-advantage filtering.
    rho_mode: str = "batch_knn"  # one of: batch_knn, constant
    rho_k: int = 10
    rho_distance_scale: float = 1.0  # <=0 uses the batch median kth-neighbor distance
    rho_power: float = 2.0
    rho_min: float = 0.0

    # Actor policy learning. The actor is updated every training step together
    # with Q and V, instead of being fit after value learning/evaluation.
    # weighted_bc is fully in-sample: weighted behavior cloning from dataset actions.
    # td3_bc uses the TD3+BC Q-improvement term with a plain BC regularizer.
    # td3_weighted_bc uses the TD3+BC Q-improvement term with advantage-weighted BC.
    ...
    # In mode="refit", Q/V are restored from --load_model and only pi is optimized.
    ...
    group: str = "SA-CDAF-JAX"
    name: str = "SA-CDAF-JAX"
    log_wandb: bool = True
    log_every: int = 100
...
def validate_config(config: TrainConfig) -> None:
    ...
    assert 0.0 <= config.beta_min <= 1.0, "beta_min must be in [0, 1]"
    assert config.rho_mode in ("batch_knn", "constant"), "rho_mode must be batch_knn or constant"
    assert config.rho_k > 0, "rho_k must be > 0"
    assert config.rho_power > 0.0, "rho_power must be > 0"
    assert 0.0 <= config.rho_min <= 1.0, "rho_min must be in [0, 1]"
    ...
    assert config.ensemble_size >= 1, "ensemble_size must be >= 1"
...
class SACDAFJAX:
    """Support-Aware Conservative Delayed Advantage Filtering for offline RL.

    SA-CDAF can use an ensemble of paired Q_i(s,a) and V_i(s) networks.
    The value target is weighted by

        w(s,a) = (1 - rho(s)) + rho(s) * beta(s,a),

    where beta is the delayed advantage filter and rho is a state-support gate.
    For ensemble_size > 1, each V_i uses Q_i as its regression target, but
    computes beta from a paired delayed head j != i. Sparse states (rho≈0)
    keep all-data SARSA-style value supervision; dense states (rho≈1) use
    filtered good-action expectation.
    """
    def __init__(
        self,
        max_action: float,
        ...
        weight_logit_clip: float = 10.0,
        beta_min: float = 0.0,
        rho_mode: str = "batch_knn",
        rho_k: int = 10,
        rho_distance_scale: float = 1.0,
        rho_power: float = 2.0,
        rho_min: float = 0.0,
        ...
        self.rho_k = rho_k
        self.rho_distance_scale = rho_distance_scale
        self.rho_power = rho_power
        ...
        def stack_ensemble_params(param_list):
        ...
        initial_filter_offset = 0 if self.ensemble_size == 1 else 1

        # Kept separately so refit mode can start pi from a fresh initialization
        # while reusing the loaded Q/V checkpoint.
        self.initial_actor_params = copy.deepcopy(actor_params)
        self.initial_actor_opt_state = self.actor_tx.init(actor_params)

        self.state = SACDAFState(
            total_it=jnp.asarray(0, dtype=jnp.int32),
            filter_rng=key_filter,
            filter_offset=jnp.asarray(initial_filter_offset, dtype=jnp.int32),
            q_params=q_params,
    ...
    def _build_train_step(self):
        ...
        def q_ensemble_apply(params: Any, states: jnp.ndarray, actions: jnp.ndarray) -> jnp.ndarray:
            return jax.vmap(lambda p: q_apply({"params": p}, states, actions))(params)

        def v_ensemble_apply(params: Any, states: jnp.ndarray) -> jnp.ndarray:
            return jax.vmap(lambda p: v_apply({"params": p}, states))(params)

        def q_ensemble_mean(params: Any, states: jnp.ndarray, actions: jnp.ndarray) -> jnp.ndarray:
            return jnp.mean(q_ensemble_apply(params, states, actions), axis=0)

        def v_ensemble_mean(params: Any, states: jnp.ndarray) -> jnp.ndarray:
            return jnp.mean(v_ensemble_apply(params, states), axis=0)

        @jax.jit
        def train_step(state: SACDAFState, actor_state: ActorState, batch: TensorBatch):
            ...
            def q_loss_fn(q_params):
                next_v_all = v_ensemble_apply(state.v_target_params, next_observations)
                target_q_all = rewards[None, :] + (1.0 - dones[None, :]) * discount * next_v_all
                q_all = q_ensemble_apply(q_params, observations, actions)
                q_loss = jnp.mean((q_all - jax.lax.stop_gradient(target_q_all)) ** 2)
                return q_loss, (q_all, target_q_all)

            (q_loss, (q_all, target_q_all)), q_grads = jax.value_and_grad(q_loss_fn, has_aux=True)(state.q_params)
            ...
            if ensemble_size == 1:
                filter_indices = jnp.zeros((1,), dtype=jnp.int32)
            else:
                filter_indices = (jnp.arange(ensemble_size, dtype=jnp.int32) + state.filter_offset) % ensemble_size

            delayed_q_all = q_ensemble_apply(state.q_delayed_params, observations, actions)
            delayed_v_all = v_ensemble_apply(state.v_delayed_params, observations)
            delayed_q_filter = jnp.take(delayed_q_all, filter_indices, axis=0)
            delayed_v_filter = jnp.take(delayed_v_all, filter_indices, axis=0)
            raw_delayed_adv = delayed_q_filter - delayed_v_filter
            ...
            # rho(s): batch-local state-support gate. rho≈0 means this state is
            # treated as sparse, so V_i(s) uses all dataset actions. rho≈1 means
            # local action comparison is considered reliable, so each V_i(s) uses
            # the delayed-advantage filter from its paired filtering head j.
            def compute_rho(states: jnp.ndarray) -> jnp.ndarray:
            ...
            rho = compute_rho(observations)
            rho_sg = jax.lax.stop_gradient(rho)[None, :]
            beta_sg = jax.lax.stop_gradient(beta)
            value_weight = (1.0 - rho_sg) + rho_sg * beta_sg

            def v_loss_fn(v_params):
                target_v_q_all = q_ensemble_apply(state.q_target_params, observations, actions)
                v_all = v_ensemble_apply(v_params, observations)
                value_residual = v_all - jax.lax.stop_gradient(target_v_q_all)
                value_loss = jnp.mean(value_weight * value_residual ** 2)
                return value_loss, (v_all, target_v_q_all)

            (value_loss, (v_all, target_v_q_all)), v_grads = jax.value_and_grad(v_loss_fn, has_aux=True)(state.v_params)
            ...
            # Actor update happens in the same training step as Q and V. The actor
            # always sees the mean of all Q/V heads, while Q/V themselves are
            # trained as paired heads (Q_i from V_i, V_i from Q_i).
            def actor_loss_fn(actor_params):
                ...
                # For actor learning, use the mean over all delayed Q/V heads.
                if actor_update_method in IQL_ACTOR_METHODS:
                    data_q = q_ensemble_mean(state.q_delayed_params, observations, actions)
                    data_v = v_ensemble_mean(state.v_delayed_params, observations)
                ...
                if actor_update_method == "weighted_bc" or actor_update_method in IQL_ACTOR_METHODS:
                    ...
                    q_pi = q_ensemble_mean(q_params, observations, pi)
            ...
            (actor_loss, (actor_q, lmbda, bc_loss, weighted_bc_loss, policy_weight_mean, policy_weight_max)), actor_grads = jax.value_and_grad(actor_loss_fn, has_aux=True)(actor_state.params)
            ...
            def refresh_filter_pairing(rng):
                if ensemble_size == 1:
                    return rng, jnp.asarray(0, dtype=jnp.int32)
                if ensemble_size == 2:
                    return rng, jnp.asarray(1, dtype=jnp.int32)
            ...
            filter_rng, filter_offset = jax.lax.cond(
                should_update_delayed,
                refresh_filter_pairing,
                lambda rng: (rng, state.filter_offset),
                state.filter_rng,
            )

            new_state = SACDAFState(
                total_it=total_it,
                filter_rng=filter_rng,
            ...
            log_dict = {
                ...
                "filter_offset": filter_offset.astype(jnp.float32),
    ...
    def _build_actor_fit_step(self):
        ...
        def actor_fit_step(
            ...
            def actor_loss_fn(actor_params):
                ...
                # Dataset-action advantage weights for weighted BC variants.
                # Refit mode freezes Q/V and updates only actor_state. All actor
                # objectives use the mean over the loaded Q/V ensemble.
                if actor_update_method in IQL_ACTOR_METHODS:
                    data_q = q_ensemble_mean(weight_q_params, observations, actions)
                    data_v = v_ensemble_mean(weight_v_params, observations)
                    data_adv = data_q - data_v
                    policy_weight = jnp.minimum(
                        jnp.exp(policy_weight_exponent * jax.lax.stop_gradient(data_adv)),
                        policy_weight_clip,
    ...
    def fit_actor(
        ...
        def save_refit_progress(is_best: bool) -> None:
        ...
        for fit_step in range(1, steps + 1):
            ...
            if should_eval:
                ...
                save_refit_progress(is_best=is_best)
    ...
    def state_dict(self) -> Dict[str, Any]:
        return {
            "sa_cdaf_state": serialization.to_state_dict(self.state),
    ...
    def _maybe_expand_legacy_ensemble_state_dict(self, loaded_state: Dict[str, Any]) -> Dict[str, Any]:
        """Make old single-head checkpoints loadable into the ensemble state.

        New checkpoints store every Q/V parameter leaf with a leading ensemble
        dimension. Older checkpoints do not. When the loaded leaf is exactly one
        leading dimension smaller than the current template, repeat it across
        ensemble heads. This also handles Adam moments for Q/V optimizers.
        """
        template_state = serialization.to_state_dict(self.state)
        loaded_state = copy.deepcopy(loaded_state)
        for key in ("filter_rng", "filter_offset"):
            if key not in loaded_state:
                loaded_state[key] = template_state[key]

        def maybe_expand(loaded_leaf, template_leaf):
            loaded_arr = np.asarray(loaded_leaf)
            template_arr = np.asarray(template_leaf)
            if (
                template_arr.ndim == loaded_arr.ndim + 1
                and template_arr.shape[0] == self.ensemble_size
                and template_arr.shape[1:] == loaded_arr.shape
            ):
                return np.repeat(loaded_arr[None, ...], self.ensemble_size, axis=0)
            return loaded_leaf

        return jax.tree_util.tree_map(maybe_expand, loaded_state, template_state)

    def load_state_dict(self, state_dict: Dict[str, Any]):
        state_key = "sa_cdaf_state" if "sa_cdaf_state" in state_dict else "cdaf_state"
        loaded_sa_state = self._maybe_expand_legacy_ensemble_state_dict(state_dict[state_key])
        self.state = serialization.from_state_dict(self.state, loaded_sa_state)
...
def resolve_checkpoint_path(
    ...
    """Return (run_dir, checkpoint_path) for a saved SA-CDAF checkpoint.
    ...
    Example:
       ...
       resolves to:
       logs/tuning/cdaf_jax/0.2/0.1/SA-CDAF-JAX-antmaze-medium-play-v2/0/checkpoint.pkl
    """
...
#     SA-CDAF uses a single value network V(s). The value target is weighted by

#         w(s,a) = (1 - rho(s)) + rho(s) * beta(s,a),

#     where beta is the delayed advantage filter and rho is a state-support gate.
#     Sparse states (rho≈0) keep all-data SARSA-style value supervision; dense
#     states (rho≈1) use filtered good-action expectation.
#     """
#     def __init__(
#         self,
#         max_action: float,
...
#             # rho(s): batch-local state-support gate. rho≈0 means this state is
#             # treated as sparse, so V(s) uses all dataset actions. rho≈1 means
#             # local action comparison is considered reliable, so V(s) uses the
#             # delayed-advantage filtered expectation.
#             def compute_rho(states: jnp.ndarray) -> jnp.ndarray:
#                 if rho_mode == "constant":
#                     return jnp.ones((states.shape[0],), dtype=states.dtype)
...
#             # Actor update happens in the same training step as Q and V. The actor
#             # sees the freshly updated Q/V parameters from this minibatch, while
#             # Q/V are not updated by the actor loss.
#             def actor_loss_fn(actor_params):
#                 pi = actor_apply({"params": actor_params}, observations)
#                 bc_error = (pi - actions) ** 2
#                 bc_per_sample = (
...
#     def load_state_dict(self, state_dict: Dict[str, Any]):
#         state_key = "sa_cdaf_state" if "sa_cdaf_state" in state_dict else "cdaf_state"
#         self.state = serialization.from_state_dict(self.state, state_dict[state_key])
#         self.actor_state = serialization.from_state_dict(self.actor_state, state_dict["actor_state"])
#         if "initial_actor_params" in state_dict:
#             self.initial_actor_params = serialization.from_state_dict(
#                 self.initial_actor_params,
...
# #     actor_fit_method: str = "td3_bc"  # one of: td3_bc, weighted_bc, td3_weighted_bc
# #     policy_weight_exponent: float = 1.0
# #     policy_weight_clip: float = 20.0
...
# #     assert config.policy_weight_exponent >= 0.0, "policy_weight_exponent must be >= 0"
# #     assert config.policy_weight_clip > 0.0, "policy_weight_clip must be > 0"
# #     assert config.actor_fit_method in (
# #         "td3_bc",
# #         "weighted_bc",
# #         "td3_weighted_bc",
# #     ), "actor_fit_method must be td3_bc, weighted_bc, or td3_weighted_bc"
# #     assert config.max_weight_exponent >= config.min_weight_exponent
# #     assert config.delayed_update_period > 0
# #     assert config.bc_coef >= 0.0
# #     assert config.batch_size > 0
...
# #         @jax.jit
# #         def train_step(state: SACDAFState, batch: TensorBatch):
# #             total_it = state.total_it + jnp.asarray(1, dtype=jnp.int32)
# #             observations = batch["observations"]
# #             actions = batch["actions"]
# #             rewards = jnp.squeeze(batch["rewards"], axis=-1)
...
# #                 # The baseline is the unified value V(s), consistent with the
# #                 # training-time delayed advantage Q_d(s,a)-V_d(s).
# #                 if actor_fit_method in ("weighted_bc", "td3_weighted_bc"):
# #                     data_q = q_apply({"params": q_params}, observations, actions)
# #                     data_v = v_apply({"params": v_params}, observations)
# #                     data_adv = jnp.clip(data_q - data_v, -weight_logit_clip, weight_logit_clip)
# #                     policy_weight = jnp.exp(policy_weight_exponent * data_adv)
...
# #                 if actor_fit_method == "weighted_bc":
# #                     actor_loss = weighted_bc_loss
# #                     q_for_log = data_q
# #                     lmbda = jnp.asarray(0.0)
# #                 else:
