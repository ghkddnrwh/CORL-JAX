...
class TrainConfig:
    ...
    # they are evaluated on a fixed dataset batch to test the paper's proposed
    # filter--error coupling mechanism.
...
class ReplayBuffer:
    def __init__(
        ...
        self._mc_returns = np.full((buffer_size, 1), np.nan, dtype=np.float32)
        ...
        for t in range(start_timestep, int(config.max_timesteps)):
            ...
            if (
                ...
                print(
                    ...
                    f"receiver_temporal_corr={diag_metrics['diag/filter_receiver_temporal_corr']:.4f}, "
