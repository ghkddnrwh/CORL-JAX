...
class CDAFJAX:
    ...
    def _build_train_step(self):
        ...
        def train_step(state: CDAFState, actor_state: ActorState, batch: TensorBatch):
            ...
            def actor_loss_fn(actor_params):
                ...
                # Dataset-action weights use delayed ensemble-mean Q/V for
                # weighted BC variants. TD3-style policy improvement below
                # still uses the freshly updated current Q ensemble.
                if actor_update_method in IQL_ACTOR_METHODS:
                    data_q = apply_q_mean(state.q_delayed_params, observations, actions)
                    data_v = apply_v_mean(state.v_delayed_params, observations)
    ...
    def _build_actor_fit_step(self):
        ...
        def actor_fit_step(
            ...
            def actor_loss_fn(actor_params):
                ...
                # Refit uses fixed learned Q/V and updates only pi. Dataset-action
                # weights use delayed ensemble-mean Q/V. TD3-style policy
                # improvement below still uses q_params.
                if actor_update_method in IQL_ACTOR_METHODS:
                    data_q = apply_q_mean(weight_q_params, observations, actions)
                    data_v = apply_v_mean(weight_v_params, observations)
                    data_adv = data_q - data_v
                    policy_weight = jnp.minimum(
                        jnp.exp(policy_weight_exponent * jax.lax.stop_gradient(data_adv)),
                        policy_weight_clip,
    ...
    def fit_actor(
        ...
        # q_params/v_params are kept for TD3-style policy improvement terms.
        # Dataset-action weights in weighted_bc, td3_weighted_bc, iql, and
        # iql_awbc are computed from delayed Q/V ensemble means.
