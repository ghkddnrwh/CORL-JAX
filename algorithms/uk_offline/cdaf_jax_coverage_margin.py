...
class CDAFJAX:
    ...
    def _build_actor_fit_step(self):
        ...
        def actor_fit_step(actor_state: ActorState, q_params: Any, v_params: Any, batch: TensorBatch):
            ...
            def actor_loss_fn(actor_params):
                ...
                # We use the learned CDAF value V(s) as the baseline, so the
                # refit-time advantage is consistent with Q(s,a)-V(s).
    ...
    def fit_actor(
        ...
        loaded_checkpoint_path: Optional[Union[str, Path]] = None,
        ...
        intermediate_save_path = Path(intermediate_save_dir) if intermediate_save_dir is not None else None
        if intermediate_save_path is not None:
            intermediate_save_path.mkdir(parents=True, exist_ok=True)

        def save_refit_progress(is_best: bool = False) -> None:
            if intermediate_save_path is None:
                return

            latest_actor_path = intermediate_save_path / "latest_actor.pkl"
            fit_logs_path = intermediate_save_path / "fit_eval_logs.npz"
            save_pickle(
                latest_actor_path,
                serialization.to_state_dict(actor_state.params),
            )

            log_row: Dict[str, Any] = {**eval_fit_log}
            if loaded_checkpoint_path is not None:
                log_row = {"loaded_checkpoint": str(loaded_checkpoint_path), **log_row}
            save_logs_npz([log_row], str(fit_logs_path))

            paths_to_upload = [latest_actor_path, fit_logs_path]
            if is_best:
                best_actor_path = intermediate_save_path / "best_actor.pkl"
                save_pickle(
                    best_actor_path,
                    serialization.to_state_dict(actor_state.params),
                )
                paths_to_upload.append(best_actor_path)

            if log_wandb and wandb.run is not None:
                for path_to_upload in paths_to_upload:
                    wandb.save(str(path_to_upload), policy="now")
        ...
        for fit_step in range(1, steps + 1):
            ...
            if should_eval:
                ...
                is_best_eval = normalized_eval_score_mean > best_normalized_score_mean
                if is_best_eval:
                ...
                save_refit_progress(is_best=is_best_eval)
...
def train(config: TrainConfig):
    ...
    if refit_only:
        ...
        print("---------------------------------------")
        print(f"Actor refit from saved {config.name} checkpoint")
        ...
        refit_actor_state, refit_log = trainer.fit_actor(
            ...
            prefix="actor_refit",
            intermediate_save_dir=actor_refit_dir,
            loaded_checkpoint_path=loaded_run_dir / "checkpoint.pkl",
